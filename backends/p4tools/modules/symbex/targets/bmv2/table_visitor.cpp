...
const IR::Expression *Bmv2V1ModelTableVisitor::computeTargetMatchType(
    ...
    if (keyProperties.matchType == BMv2Constants::MATCH_KIND_RANGE &&
        SymbexOptions::get().testBackend != "STF") {
...
bool Bmv2V1ModelTableVisitor::checkForActionProfile() {
    ...
    if (const auto *implCall = implExpr->expression->to<IR::ConstructorCallExpression>()) {
        ...
        SYMBEX_UNIMPLEMENTED("Unimplemented action profile type %1%.",
...
void Bmv2V1ModelTableVisitor::checkTargetProperties(
    const std::vector<const IR::ActionListElement *> & /*tableActionList*/) {
...
void Bmv2V1ModelTableVisitor::verifyTargetTable(
    const std::vector<const IR::ActionListElement *> &tableActionList) {
    ...
    switch (bmv2V1ModelProperties.implementaton) {
        case TableImplementation::selector: {
            if (SymbexOptions::get().testBackend != "STF") {
                // XXX
...
void Bmv2V1ModelTableVisitor::evalTargetTable(
    const std::vector<const IR::ActionListElement *> &tableActionList) {
    const auto *keys = table->getKey();
