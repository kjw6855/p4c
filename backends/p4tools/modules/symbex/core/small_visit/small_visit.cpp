...
SmallVisitEvaluator::SmallVisitEvaluator(const ProgramInfo &programInfo)
    ...
    if (!SymbexOptions::get().pattern.empty()) {
        reachabilityEngine =
            new ReachabilityEngine(programInfo.getCallGraph(), SymbexOptions::get().pattern, true);
    }
}
...
class CommandVisitor {
 ...
 public:
    Result operator()(const IR::Node *node) {
        ...
        auto *visitor =
            SymbexTarget::getCmdVisitor(state, self.get().programInfo, testCase);
        visitor->checkTable = self.get().checkTable;
        visitor->genRuleMode = self.get().genRuleMode;
    ...
    Result operator()(Continuation::Return ret) {
        if (ret.expr) {
            ...
            auto *visitor =
                SymbexTarget::getExprVisitor(state, self.get().programInfo, testCase);
            visitor->checkTable = self.get().checkTable;
            visitor->genRuleMode = self.get().genRuleMode;
