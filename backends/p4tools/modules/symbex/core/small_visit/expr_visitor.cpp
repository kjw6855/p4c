...
bool ExprVisitor::preorder(const IR::Constant *constant) {
    logStep(constant);
    return stepSymbolicValue(constant);
}

void ExprVisitor::handleHitMissActionRun(const IR::Member *member) {
...
bool ExprVisitor::preorder(const IR::Member *member) {
    logStep(member);
...
bool ExprVisitor::preorder(const IR::Mux *mux) {
    logStep(mux);
...
bool ExprVisitor::preorder(const IR::P4ValueSet *valueSet) {
    logStep(valueSet);
    ...
    SYMBEX_UNIMPLEMENTED("Value Set not yet fully implemented");
...
bool ExprVisitor::preorder(const IR::Operation_Binary *binary) {
    logStep(binary);
...
bool ExprVisitor::preorder(const IR::Operation_Unary *unary) {
    logStep(unary);
...
bool ExprVisitor::preorder(const IR::SelectExpression *selectExpression) {
    ...
    for (const auto *selectCase : selectCases) {
        ...
        if (Taint::hasTaint(matchCondition)) {
            SYMBEX_UNIMPLEMENTED(
                ...
                " Symbex currently does not support this case.",
                selectExpression);
        }
...
bool ExprVisitor::preorder(const IR::StructExpression *structExpression) {
...
bool ExprVisitor::preorder(const IR::Slice *slice) {
    logStep(slice);
