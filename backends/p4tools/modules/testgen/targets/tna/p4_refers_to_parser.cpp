...
namespace P4Tools::TnaRefersToParser {

TnaRefersToParser::TnaRefersToParser(std::vector<std::vector<const IR::Expression *>> &output)
    : restrictionsVec(output) {
    setName("TnaRefersToParser");
}
...
void TnaRefersToParser::createConstraint(bool table, cstring currentName, cstring currentKeyName,
...
void TnaRefersToParser::createRefersToConstraint(const IR::Annotation *annotation,
...
bool TnaRefersToParser::preorder(const IR::P4Table *table) {
