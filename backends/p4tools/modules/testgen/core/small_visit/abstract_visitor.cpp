...
bool AbstractVisitor::preorder(const IR::Node *node) {
    logStep(node);
    dump(node);
    BUG("%1%: Unhandled node type in %2%: %3%", node, getClassName(), node->node_type_name());
}

const ProgramInfo &AbstractVisitor::getProgramInfo() const { return programInfo; }

void AbstractVisitor::logStep(const IR::Node *node) {
...
void AbstractVisitor::checkMemberInvariant(const IR::Node *node) {
    while (true) {
...
bool AbstractVisitor::stepSymbolicValue(const IR::Node *expr) {
...
bool AbstractVisitor::stepToException(Continuation::Exception exception) {
    state.replaceTopBody(exception);
    result->emplace_back(state);
    return false;
}

bool AbstractVisitor::stepToSubexpr(
...
bool AbstractVisitor::stepToListSubexpr(
...
bool AbstractVisitor::stepToStructSubexpr(
...
bool AbstractVisitor::stepGetHeaderValidity(const IR::Expression *headerRef) {
...
void AbstractVisitor::setHeaderValidity(const IR::Expression *expr, bool validity,
...
bool AbstractVisitor::stepSetHeaderValidity(const IR::Expression *headerRef, bool validity) {
...
const IR::MethodCallStatement *generateVisitStacksetValid(const IR::Expression *stackRef, int index,
                                                     bool isValid) {
...
void generateVisitStackAssigmentStatement(ExecutionState &nextState,
    ...
    if (!value->checkedTo<IR::BoolLiteral>()->value) {
        replacements.emplace_back(generateVisitStacksetValid(stackRef, leftIndex, false));
        return;
    }
    replacements.emplace_back(generateVisitStacksetValid(stackRef, leftIndex, true));
...
bool AbstractVisitor::stepStackPushPopFront(const IR::Expression *stackRef,
    ...
    if (isPush) {
        for (int i = sz - 1; i >= 0; i -= 1) {
            if (i >= count) {
                generateVisitStackAssigmentStatement(nextState, replacements, stackRef, i, i - count);
            } else {
                replacements.emplace_back(generateVisitStacksetValid(stackRef, i, false));
            }
        }
    } else {
        for (int i = 0; i < sz; i++) {
...
void AbstractVisitor::setTargetUninitialized(ExecutionState &nextState,
...
void AbstractVisitor::declareStructLike(ExecutionState &nextState, const IR::Expression *parentExpr,
...
void AbstractVisitor::declareBaseType(ExecutionState &nextState, const IR::StateVariable &paramPath,
