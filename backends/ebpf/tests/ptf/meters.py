...
class MeterActionPSATest(P4EbpfTest):
    ...
    def runTest(self):
        ...
        self.table_add(
            table="ingress_tbl_fwd", key=[DP_PORTS[0]], action=1, data=[DP_PORTS[1]]
        )
...
class MeterPacketsPSATest(P4EbpfTest):
    ...
    def runTest(self):
        ...
        self.meter_update(
            name="ingress_meter1", index=0, pir=100, pbs=10, cir=100, cbs=10
        )
        testutils.send_packet(self, PORT0, pkt)
        testutils.verify_packet(self, pkt, PORT1)
        # Expecting pbs_left, cbs_left 10 - 1 = 9
        meter_value = build_meter_value(
            pir=100, cir=100, pbs=10, pbs_left=9, cbs=10, cbs_left=9
        )
        self.verify_map_entry(
            name="ingress_meter1",
            key="hex 00",
...
class DirectAndCounterMeterPSATest(P4EbpfTest):
    ...
    def runTest(self):
        ...
        expected_value = (
            "hex 01 00 00 00 05 00 00 00 02 00 00 00 00 00 00 00 " + meter_value
        )
        self.verify_map_entry(
            name="ingress_tbl_fwd",
            key="hex {:02x} 00 00 00".format(DP_PORTS[0]),
...
class MeterWideBitIndex(P4EbpfTest):
    ...
    def validate_meter_get(self, index):
        ...
        if (
            len(value) != 1
            or value[0]["index"]["field0"] != "0x11112222333344445555666677778888"
        ):
    ...
    def runTest(self):
        ...
        self.meter_update(
            name="ingress_meter1", index=index, pir=1, pbs=1, cir=1, cbs=1
        )
        testutils.send_packet(self, PORT0, pkt)
