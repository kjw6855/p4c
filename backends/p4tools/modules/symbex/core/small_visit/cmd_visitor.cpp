...
#include "backends/p4tools/modules/symbex/core/small_visit/table_visitor.h"
...
bool CmdVisitor::preorder(const IR::AssignmentStatement *assign) {
    ...
    if (assign->right->is<IR::StructExpression>() ||
        ...
        SYMBEX_UNIMPLEMENTED("Unsupported assignment %1% of type %2%", assign,
...
bool CmdVisitor::preorder(const IR::P4Parser *p4parser) {
    logStep(p4parser);
    ...
    for (const auto *decl : p4parser->parserLocals) {
        if (const auto *declVar = decl->to<IR::Declaration_Variable>()) {
            nextState.declareVariable(SymbexTarget::get(), *declVar);
        }
    }
...
bool CmdVisitor::preorder(const IR::P4Control *p4control) {
...
bool CmdVisitor::preorder(const IR::EmptyStatement * /*empty*/) {
    state.popBody();
    result->emplace_back(state);
    return false;
}

bool CmdVisitor::preorder(const IR::IfStatement *ifStatement) {
    logStep(ifStatement);
...
bool CmdVisitor::preorder(const IR::MethodCallStatement *methodCallStatement) {
...
bool CmdVisitor::preorder(const IR::P4Program * /*program*/) {
    ...
    auto pktSize = SymbexOptions::get().minPktSize;
    if (pktSize != 0) {
...
bool CmdVisitor::preorder(const IR::ParserState *parserState) {
    logStep(parserState);
...
bool CmdVisitor::preorder(const IR::BlockStatement *block) {
    logStep(block);
...
bool CmdVisitor::preorder(const IR::ExitStatement *e) {
    logStep(e);
    auto &nextState = state.clone();
...
const Constraint *CmdVisitor::startParser(const IR::P4Parser *parser, ExecutionState &nextState) {
...
IR::SwitchStatement *CmdVisitor::replaceSwitchLabels(const IR::SwitchStatement *switchStatement) {
...
bool CmdVisitor::preorder(const IR::SwitchStatement *switchStatement) {
