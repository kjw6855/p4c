...
#include "backends/p4tools/modules/fuzzer/core/visit_externs.h"
...
void VisitStepper::setFields(VisitState* nextState,
...
VisitStepper::PacketCursorAdvanceInfo VisitStepper::calculateSuccessfulParserAdvance(
    const VisitState& state, int advanceSize) const {
    ...
    //auto* cond = new IR::Geq(IR::Type::Boolean::get(), VisitState::getInputPacketSizeVar(),
    auto* cond = new IR::Geq(IR::Type::Boolean::get(),
                             IR::getConstant(VisitState::getPacketSizeVarType(), state.getInputPacketSize()),
                             IR::getConstant(VisitState::getPacketSizeVarType(), minSize));
    return {advanceSize, cond, advanceSize, new IR::LNot(cond)};
}

VisitStepper::PacketCursorAdvanceInfo VisitStepper::calculateAdvanceExpression(
    const VisitState& state, const IR::Expression* advanceExpr,
    const IR::Expression* restrictions) const {
    const auto* packetSizeVarType = VisitState::getPacketSizeVarType();
    ...
    auto* cond = new IR::LAnd(
        ...
        new IR::Geq(IR::Type::Boolean::get(), VisitState::getInputPacketSizeVar(), minSize));
...
void VisitStepper::generateCopyIn(VisitState* nextState, const IR::Expression* targetPath,
...
void VisitStepper::evalInternalExternMethodCall(const IR::MethodCallExpression* call,
                                               const IR::Expression* receiver, IR::ID name,
                                               const IR::Vector<IR::Argument>* args,
                                               const VisitState& state) {
    static const VisitExternMethodImpls INTERNAL_EXTERN_METHOD_IMPLS({
        /* ======================================================================================
        ...
        {"*.prepend_to_prog_header",
         ...
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            ...
            const VisitState& state, VisitStepEvaluator::VisitResult& result) {
...
void VisitStepper::evalExternMethodCall(const IR::MethodCallExpression* call,
                                       const IR::Expression* receiver, IR::ID name,
                                       const IR::Vector<IR::Argument>* args,
                                       VisitState& state) {
    // Provides implementations of all known extern methods built into P4 core.
    static const VisitExternMethodImpls CORE_EXTERN_METHOD_IMPLS({
        /* ======================================================================================
         *  packet_in.lookahead
        ...
        {"packet_in.lookahead",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             if (condInfo.advanceFailCond != nullptr) {
                 auto* rejectState = new VisitState(state);
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             if (const auto* advanceConst = advanceExpr->to<IR::Constant>()) {
                 ...
                 auto* sizeRestriction = new IR::Leq(
                     advanceExpr,
                     IR::getConstant(advanceExpr->type, VisitState::getMaxPacketLength()));
        ...
        {"packet_in.extract",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             auto receivedSize = state.getInputPacketSize();

             auto condInfo = calculateSuccessfulParserAdvance(state, extractSize);

             if (receivedSize >= extractSize)
                 condInfo.advanceFailCond = nullptr;
             else
                 condInfo.advanceCond = nullptr;
        ...
        {"packet_in.extract",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             if (const auto* varbitConst = varbitExtractExpr->to<IR::Constant>()) {
                 ...
                 // The size of the advance expression should be smaller than the maximum packet
                 // size.
                 auto maxVarbit = std::min(VisitState::getMaxPacketLength(), varbit->size);
                 auto* sizeRestriction = new IR::Leq(
        ...
        {"packet_in.length",
         {},
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            IR::ID& /*name*/, const IR::Vector<IR::Argument>* /*args*/, const VisitState& state,
            VisitStepEvaluator::VisitResult& result) {
             auto* nextState = new VisitState(state);
             const auto& lengthVar = VisitState::getInputPacketSizeVar();
             const auto* divVar =
                 new IR::Div(lengthVar->type, VisitState::getInputPacketSizeVar(),
        ...
        {"packet_out.emit",
         ...
         [](const IR::MethodCallExpression* /*call*/, const IR::Expression* /*receiver*/,
            ...
            const VisitState& state, VisitStepEvaluator::VisitResult& result) {
             ...
             {
                 auto* invalidState = new VisitState(state);
        ...
        {"*method.verify",
         ...
         [this](const IR::MethodCallExpression* call, const IR::Expression* /*receiver*/,
             ...
             if (state.hasTaint(cond)) {
                 auto* taintedState = new VisitState(state);
                 std::stringstream traceString;
             ...
             auto* falseState = new VisitState(state);
             const auto* errVar = state.getCurrentParserErrorLabel();
