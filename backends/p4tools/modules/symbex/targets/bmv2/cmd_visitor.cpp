...
void Bmv2V1ModelCmdVisitor::initializeTargetEnvironment(ExecutionState &nextState, TestCase& testCase) const {
    ...
    nextState.set(programInfo.getTargetInputPortVar(),
            IR::Constant::get(nineBitType, testCase.input_packet().port()));
    ...
    // The packet size meta data is the symbex packet length variable divided by 8.
    ...
    nextState.set(packetSizeVar, IR::Constant::get(pktSizeType, inputPacket.length()));
...
std::map<Continuation::Exception, Continuation> Bmv2V1ModelCmdVisitor::getExceptionHandlers(
