...
void DependencyGraphs::add_dependencies_from_map(size_t index, Graphs::Graph *graph,
    ...
    for (const auto &[srcVarVertex, dstVarVertices] : depEdges) {
        ...
        for (const auto &dstVarVertex : dstVarVertices) {
            ...
            if (hasFlag(srcInfo.flags, VertexFlags::CALL) &&
                    hasFlag(dstInfo.flags, VertexFlags::RETURN) &&
                ...
                //
                // Pass 1 — memoised DFS on ESG nodes to find exactly which nodes lie
                // on some srcNode→dstNode path.  Nodes on diverging branches that never
                // reach dstNode are excluded.  Back-edges are skipped to handle cycles.
                std::unordered_set<Graphs::vertex_t> onPathNodes;
                {
                    std::unordered_set<Graphs::vertex_t> dfsVisited;
                    std::unordered_set<Graphs::vertex_t> dfsStack;
                    std::function<bool(Graphs::vertex_t)> collectPaths =
                        [&](Graphs::vertex_t cur) -> bool {
                        if (cur == dstNode) return true;
                        if (dfsStack.count(cur)) return false;   // back-edge
                        if (dfsVisited.count(cur))               // memoised
                            return onPathNodes.count(cur) > 0;
                        dfsStack.insert(cur);
                        bool reaches = false;
                        for (auto next : find_next_cfg_node(graph, cur, false))
                            if (collectPaths(next)) reaches = true;
                        dfsStack.erase(cur);
                        dfsVisited.insert(cur);
                        if (reaches) onPathNodes.insert(cur);
                        return reaches;
                    };
                    collectPaths(srcNode);
                }

                // Pass 2 — DFS on dep-graph vertices to create vertices/edges,
                // restricted to onPathNodes so diverging branches are excluded.
                std::stack<vertex_t> stk;
                ...
                while (!stk.empty()) {
                    ...
                    for (auto dEsgit : nextCfgNodes) {
                        if (dEsgit == dstNode) {
                            ...
                            // Don't push dstNode; only collect nodes leading to it
                            continue;
                        }

                        // Skip nodes not on any srcNode→dstNode path
                        if (!onPathNodes.count(dEsgit)) continue;
...
DependencyGraphs::get_data_write_so_chains(size_t index, Graphs::Graph *esg,
    ...
    auto is_caller = [&](vertex_t v) -> bool {
    ...
    for (auto [so, sources] : writeSources) {
        ...
        // Run backward BFS for each write_to dst vertex
        for (auto src : sources) {
            std::deque<vertex_t> bwWork;
            std::unordered_set<vertex_t> retVertices;
            ...
            while (!bwWork.empty()) {
                ...
                for (auto [ei, ee] = boost::in_edges(v, g); ei != ee; ++ei) {
                    ...
                    // don't cross call-to-ret edges, but store target callers
                    if (g[*ei].type == DepEdgeType::CALL_TO_RET) {
                        retVertices.insert(tgt);
                        continue;
                    }

                    // skip if caller is not in the current write path
                    if (is_caller(tgt) && !retVertices.count(tgt)) {
                        continue;
                    }
        ...
        for (auto readDst : readDestinations[so]) {
            ...
            if (updateReadDstToWriteSrc.count(readDst)) {
                ...
                for (const auto &sinkDV : updateSinkDVs) {
                    auto writeVertices = filter_vertices_for_sink(writeLocalVtx, sinkDV, updateSinkDVs);
                    chains.push_back(SOChain(depGraphs[index].get(), esg,
                                                so, g[so].name, g[so].esgId.second,
            ...
            // 2) Otherwise, create a multi-execution path: pure read x write paths
                ...
                for (auto src : sources) {
                    std::vector<DependencyVertex> sinkDVs = findSinkDVs(readVtx);
                    for (const auto &sinkDV : sinkDVs) {
                        auto writeVertices = filter_vertices_for_sink(writeVtx[src], sinkDV, sinkDVs);
                        auto readVertices = filter_vertices_for_sink(readVtx, sinkDV, sinkDVs);
                        chains.push_back(SOChain(depGraphs[index].get(), esg,
                                                    so, g[so].name, g[so].esgId.second,
                                                    std::move(writeVertices),
