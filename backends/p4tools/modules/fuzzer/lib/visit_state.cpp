...
const IR::Member VisitState::inputPacketLabel =
    IR::Member(new IR::PathExpression("*"), "inputPacket");

const IR::Member VisitState::packetBufferLabel =
    IR::Member(new IR::PathExpression("*"), "packetBuffer");

const IR::Member VisitState::emitBufferLabel =
    IR::Member(new IR::PathExpression("*"), "emitBuffer");

const IR::Member VisitState::payloadLabel = IR::Member(new IR::PathExpression("*"), "payload");
...
const IR::Type_Bits VisitState::packetSizeVarType = IR::Type_Bits(32, false);

VisitState::VisitState(const IR::P4Program* program)
...
VisitState::VisitState(Continuation::Body body)
...
bool VisitState::isTerminal() const { return body.empty() && stack.empty(); }

boost::optional<const Continuation::Command> VisitState::getNextCmd() const {
    if (body.empty()) {
        return boost::none;
    }
    return body.next();
...
const IR::Expression* VisitState::get(const StateVariable& var) const {
...
void VisitState::markVisited(const IR::Statement* stmt) { visitedStatements.push_back(stmt); }

const std::vector<const IR::Statement*>& VisitState::getVisited() const {
    return visitedStatements;
}

bool VisitState::hasTaint(const IR::Expression* expr) const {
    return Taint::hasTaint(env.getInternalMap(), expr);
}

bool VisitState::exists(const StateVariable& var) const { return env.exists(var); }

void VisitState::set(const StateVariable& var, const IR::Expression* value) {
...
const SymbolicEnv& VisitState::getSymbolicEnv() const { return env; }

void VisitState::printSymbolicEnv(std::ostream& out) const {
...
const std::vector<gsl::not_null<const TraceEvent*>>& VisitState::getTrace() const {
    return trace;
}

const Continuation::Body& VisitState::getBody() const { return body; }

const std::stack<gsl::not_null<const VisitState::StackFrame*>>& VisitState::getStack()
    const {
    return stack;
}
...
bool VisitState::hasProperty(cstring propertyName) const {
    return stateProperties.count(propertyName) > 0;
}

void VisitState::addTestObject(cstring category, cstring objectLabel,
                                   const TestObject* object) {
    testObjects[category][objectLabel] = object;
}

const TestObject* VisitState::getTestObject(cstring category, cstring objectLabel,
                                                bool checked) const {
...
std::map<cstring, const TestObject*> VisitState::getTestObjectCategory(cstring category) const {
...
void VisitState::add(const TraceEvent* event) {
    CHECK_NULL(event);
...
const IR::IDeclaration* VisitState::findDecl(const IR::Path* path) const {
    return namespaces->findDecl(path);
}

const IR::IDeclaration* VisitState::findDecl(const IR::PathExpression* pathExpr) const {
    return findDecl(pathExpr->path);
}

const IR::Type* VisitState::resolveType(const IR::Type* type) const {
...
const NamespaceContext* VisitState::getNamespaceContext() const { return namespaces; }

void VisitState::setNamespaceContext(const NamespaceContext* namespaces) {
    this->namespaces = namespaces;
}

void VisitState::pushNamespace(const IR::INamespace* ns) { namespaces = namespaces->push(ns); }

void VisitState::popBody() { body.pop(); }

void VisitState::replaceBody(const Continuation::Body& body) { this->body = body; }

void VisitState::pushContinuation(gsl::not_null<const StackFrame*> frame) { stack.push(frame); }

void VisitState::pushCurrentContinuation(StackFrame::ExceptionHandlers handlers) {
    pushCurrentContinuation(boost::none, handlers);
}

void VisitState::pushCurrentContinuation(boost::optional<const IR::Type*> parameterType_opt,
...
void VisitState::popContinuation(boost::optional<const IR::Node*> argument_opt) {
...
void VisitState::handleException(Continuation::Exception e) {
    while (!stack.empty()) {
        auto frame = stack.top();
...
void VisitState::replaceTopBody(const Continuation::Command cmd) { replaceTopBody({cmd}); }

void VisitState::replaceTopBody(const std::vector<Continuation::Command>* cmds) {
...
void VisitState::replaceTopBody(std::initializer_list<Continuation::Command> cmds) {
...
void VisitState::pushPathConstraint(const IR::Expression* e) { pathConstraint.push_back(e); }

void VisitState::pushBranchDecision(uint64_t bIdx) { selectedBranches.push_back(bIdx); }

const IR::Type_Bits* VisitState::getPacketSizeVarType() { return &packetSizeVarType; }

const StateVariable& VisitState::getInputPacketSizeVar() {
    return Utils::getZombieConst(getPacketSizeVarType(), 0, "*packetLen_bits");
}

int VisitState::getMaxPacketLength() { return TestgenOptions::get().maxPktSize; }

const IR::Expression* VisitState::getInputPacket() const { return env.get(&inputPacketLabel); }

int VisitState::getInputPacketSize() const {
...
void VisitState::setInputPacketSize(int packetSize) {
    inputPacketSize = packetSize;
}

const IR::Member* VisitState::getInputPacketVar() { return &inputPacketLabel; }

const IR::Member* VisitState::getPacketBufferVar() { return &packetBufferLabel; }

void VisitState::appendToInputPacket(const IR::Expression* expr) {
    const auto* inputPkt = getInputPacket();
...
void VisitState::prependToInputPacket(const IR::Expression* expr) {
    const auto* inputPkt = getInputPacket();
    const auto* width = IR::getBitType(expr->type->width_bits() + inputPkt->type->width_bits());
...
int VisitState::getInputPacketCursor() const { return inputPacketCursor; }

const IR::Expression* VisitState::getPacketBuffer() const {
    return env.get(&packetBufferLabel);
}

int VisitState::getPacketBufferSize() const {
    const auto* buffer = getPacketBuffer();
    return buffer->type->width_bits();
}

const IR::Expression* VisitState::peekPacketBuffer(int amount) {
...
const IR::Expression* VisitState::slicePacketBuffer(int amount) {
...
void VisitState::appendToPacketBuffer(const IR::Expression* expr) {
    const auto* buffer = getPacketBuffer();
...
void VisitState::prependToPacketBuffer(const IR::Expression* expr) {
    const auto* buffer = getPacketBuffer();
    const auto* width = IR::getBitType(expr->type->width_bits() + buffer->type->width_bits());
...
void VisitState::resetPacketBuffer() {
    env.set(&packetBufferLabel, IR::getConstant(IR::getBitType(0), 0));
}

const IR::Expression* VisitState::getEmitBuffer() const { return env.get(&emitBufferLabel); }

void VisitState::resetEmitBuffer() {
    env.set(&emitBufferLabel, IR::getConstant(IR::getBitType(0), 0));
}

void VisitState::appendToEmitBuffer(const IR::Expression* expr) {
...
const IR::Member* VisitState::getPayloadLabel(const IR::Type* t) {
...
void VisitState::setParserErrorLabel(const IR::Member* parserError) {
    parserErrorLabel = parserError;
}

const IR::Member* VisitState::getCurrentParserErrorLabel() const {
...
const std::set<StateVariable>& VisitState::getZombies() const { return allocatedZombies; }

const StateVariable& VisitState::createZombieConst(const IR::Type* type, cstring name,
                                                       uint64_t instanceId) {
...
/* =========================================================================================
 *  General utilities involving VisitState.
 * ========================================================================================= */

std::vector<const IR::Member*> VisitState::getFlatFields(
...
const IR::P4Table* VisitState::getTableType(const IR::Expression* expression) const {
...
const IR::P4Action* VisitState::getActionDecl(const IR::Expression* expression) const {
...
const StateVariable& VisitState::convertPathExpr(const IR::PathExpression* path) const {
