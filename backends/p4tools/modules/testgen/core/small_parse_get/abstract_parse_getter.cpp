...
AbstractParseGetter::AbstractParseGetter(ExecutionState &state,
                                 const ProgramInfo &programInfo)
    : programInfo(programInfo), state(state), result(new std::vector<Branch>()) {}

AbstractParseGetter::Result AbstractParseGetter::parse_get(const IR::Node *node) {
    node->apply(*this);
    return result;
}

bool AbstractParseGetter::preorder(const IR::Node *node) {
    logParseGet(node);
    dump(node);
    BUG("%1%: Unhandled node type in %2%: %3%", node, getClassName(), node->node_type_name());
}

const ProgramInfo &AbstractParseGetter::getProgramInfo() const { return programInfo; }

void AbstractParseGetter::logParseGet(const IR::Node *node) {
    LOG_FEATURE("small_parse_get", 4,
                "***** " << getClassName() << "(" << node->node_type_name() << ") *****"
                         << std::endl
                         << node << std::endl);
}

void AbstractParseGetter::checkMemberInvariant(const IR::Node *node) {
    while (true) {
...
bool AbstractParseGetter::parseGetSymbolicValue(const IR::Node *expr) {
...
bool AbstractParseGetter::parseGetToException(Continuation::Exception exception) {
    state.replaceTopBody(exception);
    result->emplace_back(state);
    return false;
}

bool AbstractParseGetter::parseGetToSubexpr(
    const IR::Expression *subexpr, SmallParseGetEvaluator::Result &result, const ExecutionState &state,
...
bool AbstractParseGetter::parseGetToListSubexpr(
    const IR::ListExpression *subexpr, SmallParseGetEvaluator::Result &result,
    ...
    // parameter.
    // XXX This results in the small-parse_get evaluator evaluating list expressions in quadratic time.
...
bool AbstractParseGetter::parseGetToStructSubexpr(
    const IR::StructExpression *subexpr, SmallParseGetEvaluator::Result &result,
    const ExecutionState &state,
    std::function<const Continuation::Command(const IR::StructExpression *)> rebuildCmd) {
    // @see parseGetToListSubexpr regarding the assessment of complexity of this function.
...
bool AbstractParseGetter::parse_getGetHeaderValidity(const IR::Expression *headerRef) {
...
void AbstractParseGetter::setHeaderValidity(const IR::Expression *expr, bool validity,
...
bool AbstractParseGetter::parse_getSetHeaderValidity(const IR::Expression *headerRef, bool validity) {
...
const IR::MethodCallStatement *generateParseStacksetValid(const IR::Expression *stackRef, int index,
                                                     bool isValid) {
...
void generateParseStackAssigmentStatement(ExecutionState &nextState,
    ...
    if (!value->checkedTo<IR::BoolLiteral>()->value) {
        replacements.emplace_back(generateParseStacksetValid(stackRef, leftIndex, false));
        return;
    }
    replacements.emplace_back(generateParseStacksetValid(stackRef, leftIndex, true));
...
bool AbstractParseGetter::parseGetStackPushPopFront(const IR::Expression *stackRef,
    ...
    if (isPush) {
        for (int i = sz - 1; i >= 0; i -= 1) {
            if (i >= count) {
                generateParseStackAssigmentStatement(nextState, replacements, stackRef, i, i - count);
            } else {
                replacements.emplace_back(generateParseStacksetValid(stackRef, i, false));
            }
        }
    } else {
        for (int i = 0; i < sz; i++) {
...
const IR::Literal *AbstractParseGetter::evaluateExpression(
...
void AbstractParseGetter::setTargetUninitialized(ExecutionState &nextState,
...
void AbstractParseGetter::declareStructLike(ExecutionState &nextState, const IR::Expression *parentExpr,
...
void AbstractParseGetter::declareBaseType(ExecutionState &nextState, const IR::StateVariable &paramPath,
