...
TnaCmdStepper::TnaCmdStepper(ExecutionState &state, AbstractSolver &solver,
                                             const ProgramInfo &programInfo)
    : CmdStepper(state, solver, programInfo) {}

const TnaProgramInfo &TnaCmdStepper::getProgramInfo() const {
    return *CmdStepper::getProgramInfo().checkedTo<TnaProgramInfo>();
}

void TnaCmdStepper::initializeTargetEnvironment(ExecutionState &nextState) const {
    ...
    // TNA initializes all metadata to zero. To avoid unnecessary taint, we retrieve the type and
    ...
    nextState.set(programInfo.getTargetInputPortVar(),
                  nextState.createSymbolicVariable(nineBitType, "tna_ingress_port"));
    // TNA implicitly sets the output port to 0.
...
std::optional<const Constraint *> TnaCmdStepper::startParserImpl(
    const IR::P4Parser *parser, ExecutionState &nextState) const {
    // We need to explicitly map the parser error
    const auto *errVar = TnaProgramInfo::getParserParamVar(
...
std::map<Continuation::Exception, Continuation> TnaCmdStepper::getExceptionHandlers(
