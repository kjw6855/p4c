...
def plot_preconditions():
    ...
    target_frame = pd.DataFrame(
        {"Precondition": preconditions, "Number of tests": data}
    )
...
def get_strategy_data(input_dir):
    ...
    for folder in folders:
        ...
        for strategy_file in strategy_files:
            m = re.search(
                f"{program_name}_(.+?)_coverage_over_time", strategy_file.stem
            )
            if m:
...
def plot_strategies(args, extra_args):
    ...
    for strategy, candidate_data in program_data.items():
        ...
        candidate_data.Minutes = pd.to_timedelta(
            candidate_data.Minutes, unit="nanoseconds"
        )
    ...
    ax = sns.lineplot(
        x="Minutes", y="Coverage", hue="Strategy", data=concat_data, errorbar="sd"
    )
