...
#include "backends/p4tools/modules/fuzzer/core/constants.h"
...
const VisitState* TableVisitor::getVisitState() { return &stepper->state; }

const ProgramInfo* TableVisitor::getProgramInfo() { return &stepper->programInfo; }

VisitStepper::VisitResult TableVisitor::getResult() { return stepper->result; }

const StateVariable& TableVisitor::getTableActionVar(const IR::P4Table* table) {
...
const StateVariable& TableVisitor::getTableHitVar(const IR::P4Table* table) {
    return Utils::getZombieTableVar(IR::Type::Boolean::get(), table, "*hit");
}

const StateVariable& TableVisitor::getTableKeyReadVar(const IR::P4Table* table, int keyIdx) {
    const auto* key = table->getKey()->keyElements.at(keyIdx);
    return Utils::getZombieTableVar(key->expression->type, table, "*keyRead", keyIdx);
}

const StateVariable& TableVisitor::getTableReachedVar(const IR::P4Table* table) {
    return Utils::getZombieTableVar(IR::Type::Boolean::get(), table, "*reached");
}

bool TableVisitor::compareLPMEntries(const IR::Entry* leftIn, const IR::Entry* rightIn,
                                     size_t lpmIndex) {
...
const IR::Expression* TableVisitor::computeTargetMatchType(
    VisitState* nextState, const KeyProperties& keyProperties,
...
const IR::Expression* TableVisitor::computeHit(VisitState* nextState,
...
bool TableVisitor::resolveTableKeys() {
    ...
    const auto* state = getVisitState();
...
void TableVisitor::checkTableIsImmutable() {
...
std::vector<const IR::ActionListElement*> TableVisitor::buildTableActionList() {
...
void TableVisitor::addDefaultAction(boost::optional<const IR::Expression*> tableMissCondition) {
