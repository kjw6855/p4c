...
class RunBMV2(object):
    def __init__(self, folder, options, jsonfile):
        ...
        self.target_specific_cmd_line_args = getattr(
            options, "switchTargetSpecificOptions", ()
        )
    ...
    def do_command(self, cmd):
        ...
        if first == "":
            ...
            first == "mc_mgrp_create"
            or first == "mc_node_create"
            or first == "mc_node_associate"
        ):
            ...
            first == "register_read"
            or first == "register_write"
            or first == "register_reset"
        ):
        ...
        for name, id in table.actions.items():
            ...
            if action.name.endswith(actionName):
                if candidate is None:
                    ...
                    raise Exception(
                        "Ambiguous action name " + actionName + " in " + table.name
                    )
    ...
    def interfaceArgs(self):
        ...
        for interface in sorted(self.interfaces):
            result.append(
                "-i " + str(interface) + "@" + self.pcapPrefix + str(interface)
            )
        return result
            ...
            for line in i:
                ...
                if first == "packet" or first == "expect":
                    ...
                    if not interface in self.interfaces:
                        ...
                        ifname = self.interfaces[interface] = self.filename(
                            interface, "in"
                        )
                        os.mkfifo(ifname)
        return SUCCESS
    ...
    def checkOutputs(self):
        ...
        for file in glob(self.filename("*", direction)):
            ...
            if len(expected) != len(packets):
                ...
                reportError(
                    "Full list of %d expected packets on port %d:"
                ...
                reportError(
                    "Full list of %d received packets on port %d:"
