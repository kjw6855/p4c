...
std::vector<std::vector<Continuation::Command>> JBayProgramInfo::ingressCmds() const {
    return pipelineCmds(INGRESS);
}

std::vector<std::vector<Continuation::Command>> JBayProgramInfo::egressCmds() const {
    return pipelineCmds(EGRESS);
}

std::optional<const IR::Expression *> JBayProgramInfo::getPipePortRangeConstraint(
    ...
    if (pipes.size() == 2) {
        ...
        // The upper bit determines the pipe.
        size_t pipeScope = pipeIdx << 1;
        return new IR::LOr(
            new IR::Equ(pipeNumber, new IR::Constant(pipeNumber->type, pipeScope)),
            new IR::Equ(pipeNumber, new IR::Constant(pipeNumber->type, pipeScope | 0x1)));
    }
    if (pipes.size() > 2) {
...
const IR::Expression *JBayProgramInfo::getValidPortConstraint(
    const IR::StateVariable &portVar) const {
    // const auto *portNumber = new IR::Slice(portVar, 6, 0);
    const auto &options = SymbexOptions::get();

    // 0-7 are PCIe/CPU/recirc/pktgen ports
    // auto *validMacPort =
    //     new IR::LAnd(new IR::Leq(portNumber, new IR::Constant(portNumber->type, 0x47)),
    //                  new IR::Leq(new IR::Constant(portNumber->type, 0x8), portNumber));
    // auto *allowedPort =
...
std::vector<std::vector<Continuation::Command>> JBayProgramInfo::pipelineCmds(gress_t gress) const {
    ...
    for (const auto &pipeInfo : pipes) {
        ...
        /// We are missing the ghost thread in this particular program (it is optional)
        // Just advance the pipeIdx to circumvent the check.
        // This is only allowed in the case of 6 pipes.
        if (blockIdx == 6) {
            ++blockIdx;
        }
        BUG_CHECK(archSpec.getArchVectorSize() >= blockIdx,
                  "The Tofino architecture requires at most %1% pipes (provided %2% pipes).",
                  archSpec.getArchVectorSize(), blockIdx);
    }
    return pipelineSequences;
}

JBayProgramInfo::JBayProgramInfo(const TofinoCompilerResult &compilerResult,
...
std::vector<Continuation::Command> JBayProgramInfo::processDeclaration(
    const IR::Type_Declaration *typeDecl, size_t pipeIdx) const {
    // Get the architecture specification for this target.
    const auto &archSpec = getArchSpec();
    const auto &program = getCompilerResult().getProgram();
    ...
    if ((archMember->blockName == "IngressParserT")) {
        ...
        const auto *portMetadata =
            createTargetUninitialized(IR::Type_Bits::get(JBayConstants::PORT_METADATA_SIZE), false);
        const auto *stmt = new IR::MethodCallStatement(Utils::generateInternalMethodCall(
            ...
            new IR::ParameterList(
                {new IR::Parameter("hdr", IR::Direction::In,
                                   IR::Type_Bits::get(JBayConstants::PORT_METADATA_SIZE))})));
        ...
        const auto *paramType =
            resolveProgramType(&program, new IR::Type_Name("ingress_intrinsic_metadata_t"));
    ...
    if ((archMember->blockName == "EgressParserT")) {
        const auto *paramType =
            resolveProgramType(&program, new IR::Type_Name("egress_intrinsic_metadata_t"));
...
const ArchSpec &JBayProgramInfo::getArchSpec() const { return ARCH_SPEC; }

const ArchSpec JBayProgramInfo::ARCH_SPEC = ArchSpec(
    "Switch"_cs,
    {
        ...
        {"IngressT"_cs,
         {"*ig_hdr"_cs, "*ig_md"_cs, "*ig_intr_md"_cs, "*ig_intr_md_from_prsr"_cs,
          "*ig_intr_md_for_dprsr"_cs, "*ig_intr_md_for_tm"_cs, "*gh_intr_md"_cs}},
