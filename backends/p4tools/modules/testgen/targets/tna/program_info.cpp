...
#include "backends/p4tools/modules/testgen/targets/tna/concolic.h"
#include "backends/p4tools/modules/testgen/targets/tna/constants.h"
#include "backends/p4tools/modules/testgen/targets/tna/map_direct_externs.h"
#include "backends/p4tools/modules/testgen/targets/tna/p4_asserts_parser.h"
#include "backends/p4tools/modules/testgen/targets/tna/p4_refers_to_parser.h"

namespace P4Tools::P4Testgen::Tna {

const IR::Type_Bits TnaProgramInfo::PARSER_ERR_BITS = IR::Type_Bits(32, false);

TnaProgramInfo::TnaProgramInfo(
    ...
    concolicMethodImpls.add(*TnaConcolic::getTnaConcolicMethodImpls());
    ...
    BUG_CHECK(archSpec->getArchVectorSize() == programmableBlocks.size(),
              "The TNA architecture requires %1% pipes (provided %2% pipes).",
    ...
    /// Sending a too short packet in TNA produces nonsense, so we require the packet size to be
    /// larger than 32 bits.This number needs to be raised to the size of the ethernet header for
    /// the PTF and PROTOBUF back ends.
    auto minPktSize = TNAConstants::STF_MIN_PKT_SIZE;
    if (options.testBackend == "PTF") {
        minPktSize = TNAConstants::ETH_HDR_SIZE;
    }
    ...
    program->apply(TnaAssertsParser::TnaAssertsParser(restrictionsVec));
    ...
    program->apply(TnaRefersToParser::TnaRefersToParser(restrictionsVec));
...
const IR::P4Table *TnaProgramInfo::getTableofDirectExtern(
...
const ordered_map<cstring, const IR::Type_Declaration *>
    *TnaProgramInfo::getProgrammableBlocks() const {
    return &programmableBlocks;
}

int TnaProgramInfo::getGress(const IR::Type_Declaration *decl) const {
    return declIdToGress.at(decl->declid);
}

std::vector<Continuation::Command> TnaProgramInfo::processDeclaration(
    ...
    if ((archMember->blockName == "Ingress")) {
        auto *egressPortVar =
            new IR::Member(IR::getBitType(TNAConstants::PORT_BIT_WIDTH),
...
const IR::StateVariable &TnaProgramInfo::getTargetInputPortVar() const {
    return *new IR::StateVariable(new IR::Member(IR::getBitType(TNAConstants::PORT_BIT_WIDTH),
                                                 new IR::PathExpression("*standard_metadata"),
                                                 "ingress_port"));
}

const IR::Expression *TnaProgramInfo::getOutPortConstraint(const IR::StateVariable &portVar,
        int minPortNo, int maxPortNo, std::vector<int> allowPorts) {
    const IR::Operation_Binary *portConstraint =
        new IR::LOr(new IR::Equ(portVar, new IR::Constant(portVar->type, TNAConstants::DROP_PORT)),
                    new IR::LAnd(
...
const IR::Expression *TnaProgramInfo::getInPortConstraint(const IR::StateVariable &portVar,
        int minPortNo, int maxPortNo, std::vector<int> allowPorts) {
    const IR::Operation_Binary *portConstraint =
        new IR::LAnd(new IR::Neq(portVar, new IR::Constant(portVar->type, TNAConstants::DROP_PORT)),
                    new IR::LAnd(
                        new IR::Geq(portVar, new IR::Constant(portVar->type, minPortNo)),
                        new IR::Lss(portVar, new IR::Constant(portVar->type, maxPortNo))));
...
const IR::StateVariable &TnaProgramInfo::getTargetOutputPortVar() const {
    return *new IR::StateVariable(new IR::Member(IR::getBitType(TNAConstants::PORT_BIT_WIDTH),
                                                 new IR::PathExpression("*standard_metadata"),
                                                 "egress_spec"));
}

const IR::Expression *TnaProgramInfo::dropIsActive() const {
    const auto &egressPortVar = getTargetOutputPortVar();
    return new IR::Equ(IR::getConstant(egressPortVar->type, TNAConstants::DROP_PORT),
                       egressPortVar);
}

const IR::Type_Bits *TnaProgramInfo::getParserErrorType() const { return &PARSER_ERR_BITS; }

const IR::PathExpression *TnaProgramInfo::getBlockParam(cstring blockLabel,
...
const IR::Member *TnaProgramInfo::getParserParamVar(const IR::P4Parser *parser,
