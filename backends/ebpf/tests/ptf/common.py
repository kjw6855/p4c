...
class P4EbpfTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.exec_ns_cmd(
            "nikss-ctl pipeline load id {} {}".format(
                TEST_PIPELINE_ID, self.test_prog_image
            ),
    ...
    def exec_cmd(self, command, do_fail=None):
        ...
        if process.returncode != 0:
            ...
            if do_fail:
                self.fail(
                    "Command failed (see above for details): {}".format(str(do_fail))
                )
        return process.returncode, stdout_data, stderr_data

    def add_port(self, dev):
        self.exec_ns_cmd(
            "nikss-ctl add-port pipe {} dev {}".format(TEST_PIPELINE_ID, dev)
        )
    ...
    def del_port(self, dev):
        self.exec_ns_cmd(
            "nikss-ctl del-port pipe {} dev {}".format(TEST_PIPELINE_ID, dev)
        )
        if dev.startswith("eth"):
            self.exec_cmd(
                "nikss-ctl del-port pipe {} dev s1-{}".format(TEST_PIPELINE_ID, dev)
            )
    ...
    def multicast_group_create(self, group):
        self.exec_ns_cmd(
            "nikss-ctl multicast-group create pipe {} id {}".format(
                TEST_PIPELINE_ID, group
            )
        )
    ...
    def multicast_group_delete(self, group):
        self.exec_ns_cmd(
            "nikss-ctl multicast-group delete pipe {} id {}".format(
                TEST_PIPELINE_ID, group
            )
        )
    ...
    def table_write(
        ...
        cmd = cmd + self._table_create_str_from_data(
            data=data, counters=counters, meters=meters
        )
        if priority:
    ...
    def table_verify(
        ...
        if action is not None:
            if action != entry["action"]["id"]:
                self.fail(
                    "Invalid action ID: expected {}, got {}".format(
        ...
        if priority is not None:
            if priority != entry["priority"]:
                self.fail(
                    "Invalid priority: expected {}, got {}".format(
    ...
    def action_selector_add_action(self, selector, action, data=None):
        cmd = "nikss-ctl action-selector add-member pipe {} {} ".format(
            TEST_PIPELINE_ID, selector
        )
        cmd = cmd + self._table_create_str_from_action(action)
        cmd = cmd + self._table_create_str_from_data(
            data=data, counters=None, meters=None
        )
    ...
    def action_selector_create_empty_group(self, selector):
        cmd = "nikss-ctl action-selector create-group pipe {} {}".format(
            TEST_PIPELINE_ID, selector
        )
    ...
    def action_profile_add_action(self, ap, action, data=None):
        cmd = "nikss-ctl action-profile add-member pipe {} {} ".format(
            TEST_PIPELINE_ID, ap
        )
        cmd = cmd + self._table_create_str_from_action(action)
        cmd = cmd + self._table_create_str_from_data(
    ...
    def counter_get(self, name, key=None):
        ...
        cmd = "nikss-ctl counter get pipe {} {} {}".format(
            TEST_PIPELINE_ID, name, key_str
        )
    ...
    def _do_counter_verify(self, bytes, packets, entry_value, counter_type):
        ...
        if expected_type != counter_type:
            self.fail(
                'Invalid counter type, expected: "{}", got "{}"'.format(
        ...
        if bytes is not None:
            ...
            if counter_bytes != bytes:
                self.fail(
                    "Invalid counter bytes, expected {}, got {}".format(
                        bytes, counter_bytes
                    )
                )
        if packets is not None:
            ...
            if counter_packets != packets:
                self.fail(
                    "Invalid counter packets, expected {}, got {}".format(
    ...
    def register_get(self, name, index=None):
        ...
        cmd = "nikss-ctl register get pipe {} {} {}".format(
            TEST_PIPELINE_ID, name, index_str
        )
