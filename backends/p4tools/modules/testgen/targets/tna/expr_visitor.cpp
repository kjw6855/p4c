...
#include "backends/p4tools/modules/testgen/targets/tna/table_visitor.h"
...
std::string TnaExprVisitor::getClassName() { return "TnaExprVisitor"; }

bool TnaExprVisitor::isPartOfFieldList(const IR::StructField *field,
...
void TnaExprVisitor::resetPreservingFieldList(ExecutionState &nextState,
...
void TnaExprVisitor::processClone(const ExecutionState &state,
                                          SmallStepEvaluator::Result &result) {
    const auto *progInfo = getProgramInfo().checkedTo<TnaProgramInfo>();
    // Pick a clone port var. It must adhere to the constraints of the target.
    const auto *cloneInfo = state.getTestObject<TnaCloneInfo>("clone_infos", "clone_info");
        ...
        cond = new IR::LAnd(
            new IR::LAnd(
                new IR::Leq(sessionIdExpr, IR::getConstant(sessionIdExpr->type,
                                                           TNAConstants::CLONE_SESSION_ID_MAX)),
                new IR::Geq(sessionIdExpr, IR::getConstant(sessionIdExpr->type,
                                                           TNAConstants::CLONE_SESSION_ID_MIN))),
    ...
    // clone methods have a default state where the packet continues as is.
    {
        ...
        const auto *defaultCloneInfo = new TnaCloneSpec(sessionIdExpr, clonePortVar, false);
    ...
    if (cloneType == TNAConstants::CloneType::I2E) {
        ...
        cloneState->set(
            instanceTypeVar,
            IR::getConstant(instanceBitType, TNAConstants::PKT_INSTANCE_TYPE_INGRESS_CLONE));
        const auto *progInfo = getProgramInfo().checkedTo<TnaProgramInfo>();
    ...
    } else if (cloneType == TNAConstants::CloneType::E2E) {
        cloneState = &state.clone();
        // Set the metadata instance type.
        cloneState->set(
            instanceTypeVar,
            IR::getConstant(instanceBitType, TNAConstants::PKT_INSTANCE_TYPE_EGRESS_CLONE));
        ...
        for (; egressDelim < topLevelBlocks->size(); ++egressDelim) {
            ...
            if (const auto *ctrl = p4Node->to<IR::P4Control>()) {
                if (progInfo->getGress(ctrl) == TNA_EGRESS) {
                    break;
                }
            }
        }
    ...
    cloneState->addTestObject("clone_specs", "clone_spec",
                              new TnaCloneSpec(sessionIdExpr, clonePortVar, true));
    // Delete the stale clone info to free up some space and reset the clone_active flag.
...
void TnaExprVisitor::processRecirculate(const ExecutionState &state,
                                                SmallStepEvaluator::Result &result) {
...
TnaExprVisitor::TnaExprVisitor(ExecutionState &state,
                                               const ProgramInfo &programInfo, TestCase &testCase)
    : ExprVisitor(state, programInfo, testCase) {}

void TnaExprVisitor::evalExternMethodCall(const IR::MethodCallExpression *call,
    ...
    // Provides implementations of TNA externs.
    static const ExternMethodImpls EXTERN_METHOD_IMPLS({
        /* ======================================================================================
         *  mark_to_drop
         *  Mark to drop sets the TNA internal drop variable to true.
         * ======================================================================================
         */
        ...
        {"*method.mark_to_drop",
         ...
         [](const IR::MethodCallExpression * /*call*/, const IR::Expression * /*receiver*/,
            ...
            const ExecutionState &state, SmallStepEvaluator::Result &result) {
             auto &nextState = state.clone();
             const auto *nineBitType = IR::getBitType(TNAConstants::PORT_BIT_WIDTH);
             ...
             nextState.set(portVar, IR::getConstant(nineBitType, TNAConstants::DROP_PORT));
        ...
        {"register.read",
         ...
         [this](const IR::MethodCallExpression *call, const IR::Expression *receiver,
             ...
             const TnaRegisterValue *registerValue = nullptr;
             if (registerState != nullptr) {
                 registerValue = registerState->checkedTo<TnaRegisterValue>();
             } else {
                 const auto *inputValue =
                     programInfo.createTargetUninitialized(readOutput->type, false);
                 registerValue = new TnaRegisterValue(inputValue);
        ...
        {"register.write",
         ...
         [this](const IR::MethodCallExpression *call, const IR::Expression *receiver,
             ...
             TnaRegisterValue *registerValue = nullptr;
             if (registerState != nullptr) {
                 registerValue = new TnaRegisterValue(
                     *registerState->checkedTo<TnaRegisterValue>());
                 ...
                 registerValue = new TnaRegisterValue(writeValue);
                 registerValue->writeToIndex(index, inputValue);
             }
             nextState.addTestObject("registervalues", externInstance->controlPlaneName(),
                                     registerValue);
            ...
            const ExecutionState &state, SmallStepEvaluator::Result &result) {
             ...
             if (testBackend != "PTF") {
                 ...
                 nextState.set(meterResult, IR::getConstant(meterResult->type,
                                                            TNAConstants::METER_COLOR::GREEN));
                 nextState.popBody();
                 result->emplace_back(nextState);
                 return;
             }
             ...
             TnaMeterValue *meterValue = nullptr;
             ...
             if (meterState != nullptr) {
                 meterValue =
                     new TnaMeterValue(*meterState->checkedTo<TnaMeterValue>());
             } else {
                 meterValue = new TnaMeterValue(inputValue, false);
             }
        ...
        {"direct_meter.read",
         ...
         [this](const IR::MethodCallExpression *call, const IR::Expression *receiver,
             ...
             if (meterState != nullptr) {
                 ...
                 meterValue = new TnaMeterValue(inputValue, true);
             }
         ...
         *  control plane API.
         *
         *  The TNA implementation of the v1model architecture ignores the
        ...
        {"*method.clone_preserving_field_list",
         ...
         [](const IR::MethodCallExpression *call, const IR::Expression * /*receiver*/,
            ...
            const ExecutionState &state, SmallStepEvaluator::Result &result) {
             ...
             auto *cloneInfo = new TnaCloneInfo(
                 sessionIdExpr, static_cast<TNAConstants::CloneType>(cloneType), state.clone(),
                 preserveIndex);
             auto &nextState = state.clone();
         ...
         [](const IR::MethodCallExpression * /*call*/, const IR::Expression * /*receiver*/,
            ...
            const ExecutionState &state, SmallStepEvaluator::Result &result) {
             ...
             nextState.setProperty("recirculate_instance_type",
                                   TNAConstants::PKT_INSTANCE_TYPE_RESUBMIT);
             nextState.popBody();
             result->emplace_back(nextState);
         }},
        /* ======================================================================================
        ...
        {"*method.recirculate_preserving_field_list",
         ...
         [](const IR::MethodCallExpression * /*call*/, const IR::Expression * /*receiver*/,
            ...
            const ExecutionState &state, SmallStepEvaluator::Result &result) {
             ...
             // "processRecirculate".
             nextState.setProperty("recirculate_instance_type",
                                   TNAConstants::PKT_INSTANCE_TYPE_RECIRC);
             nextState.popBody();
             result->emplace_back(nextState);
         }},
        /* ======================================================================================
    ...
    if (!EXTERN_METHOD_IMPLS.exec(call, receiver, name, args, state, result)) {
        ExprVisitor::evalExternMethodCall(call, receiver, name, args, state);
    }
}  // NOLINT

bool TnaExprVisitor::preorder(const IR::P4Table *table) {
    // Delegate to the tableVisitor.
    TnaTableVisitor tableVisitor(this, table, testCase);
