...
#include "backends/p4tools/modules/symbex/test/z3-solver/accessor.h"
...
class Z3SolverTest : public SymbexBmv2Test {
 protected:
    void SetUp() override {
        SymbexBmv2Test::SetUp();
        opLss = nullptr;
      ...
      V1Switch(parse(), verifyChecksum(), mau(), mau(), computeChecksum(), deparse()) main;)");
        ...
        const auto *progInfo = SymbexTarget::produceProgramInfo(test->getCompilerResult());
        if (progInfo == nullptr) {
            return;
        }
