#ifndef BACKENDS_P4TOOLS_MODULES_TESTGEN_CORE_SMALL_PARSE_GET_ABSTRACT_PARSE_GETTER_H_
#define BACKENDS_P4TOOLS_MODULES_TESTGEN_CORE_SMALL_PARSE_GET_ABSTRACT_PARSE_GETTER_H_
...
/// A framework for implementing small-parse_get operational semantics. Each instance is good for one
/// small-parse_get evaluation.
///
...
class AbstractParseGetter : public Inspector {
 public:
    using Branch = SmallStepEvaluator::Branch;
    using Result = SmallStepEvaluator::Result;

    /// ParseGets on the given node. This is the main entry point into a parse_getter.
    ///
    ...
    Result parse_get(const IR::Node *);

    /// Provides generic handling of unsupported nodes.
    bool preorder(const IR::Node *) override;

    AbstractParseGetter(ExecutionState &state, const ProgramInfo &programInfo);

 protected:
    struct TransitionCond {
        size_t startBit;
        size_t bitLen;
        big_int value;
    };

    struct TransitionEdgeWithCond {
        const IR::ParserState *sourceState;
        const IR::ParserState *destState;
        std::vector<const TransitionCond *> conditions;
    };
    ...
    /// Helper function for debugging execution of small parse_getter.
    void logParseGet(const IR::Node *node);
    ...
    ///
    /// @returns false
    bool parseGetSymbolicValue(const IR::Node *);

    /// Transitions to an exception.
    /// @returns false
    bool parseGetToException(Continuation::Exception);
    ...
    /// parseGetToSubExpr that can be used in contexts that explicitly expect a ListExpression instead
    ...
    ///
    /// @returns false
    static bool parseGetToListSubexpr(
    ...
    /// @see parseGetToListSubexpr.IR::StructExpression differs slightly from IR::ListExpression in that
    ...
    static bool parseGetToStructSubexpr(
    ...
    ///
    /// @returns false
    bool parse_getGetHeaderValidity(const IR::Expression *headerRef);
    ...
    ///
    /// @returns false
    bool parse_getSetHeaderValidity(const IR::Expression *headerRef, bool validity);
    ...
    ///
    /// @returns false
    bool parseGetStackPushPopFront(const IR::Expression *stackRef, const IR::Vector<IR::Argument> *args,
                               bool isPush = true);
...
#endif /* BACKENDS_P4TOOLS_MODULES_TESTGEN_CORE_SMALL_PARSE_GET_ABSTRACT_PARSE_GETTER_H_ */
