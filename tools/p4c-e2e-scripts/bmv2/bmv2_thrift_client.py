...
class Bmv2ThriftClient:
    ...
    def write_register(self, register_name: str, index: int, value: int) -> None:
        """Set one cell of a register array."""
        self._client.bm_register_write(self.CTX_ID, register_name, index, value)

    def write_register_all(self, register_name: str, value: int) -> None:
        """Set every cell of a register array to @value.

        Needed because replay must establish the state symbex assumed: a declared non-zero
        initial value applies to every cell, and clearing to zero leaves them disagreeing.
        The array size is discovered by reading it back, since the Thrift API exposes no size
        query.
        """
        try:
            size = len(self._client.bm_register_read_all(self.CTX_ID, register_name))
        except Exception:
            size = 0
        for idx in range(size):
            self._client.bm_register_write(self.CTX_ID, register_name, idx, value)
