...
class UPFTest(P4EbpfTest):
    ...
    def setup_pfcp_session(self, seid, teid, ue_ip):
        self.table_add(
            table="ingress_upf_ingress_session_lookup_by_teid",
            key=[teid],
            action=1,
            data=[seid],
        )
        self.table_add(
            table="ingress_upf_ingress_session_lookup_by_ue_ip",
            key=[ue_ip],
            action=1,
            data=[seid],
        )
    ...
    def runTest(self):
        ...
        pkt = testutils.simple_udp_packet(
            eth_dst=UPF_N6_MAC, ip_src=SERVER1_IP, ip_dst=UE_IP
        )
        exp_pkt = pkt.copy()
        exp_pkt[IP].ttl -= 1
