...
SYMBEX_BIN = FILE_DIR.joinpath("../../../../../build/symbex")
...
PARSER.add_argument(
    ...
    dest="symbex_bin",
    default=SYMBEX_BIN,
    help="Specifies the symbex binary.",
)
PARSER.add_argument(
    "-t",
    "--test-backend",
...
class Options:
    ...
    # The architecture.
    arch = "v1model"
    # Extra arguments for Symbex execution.
    extra_args = None
...
def run_strategies_for_max_tests(options, test_args):
    cmd = (
        f"{options.symbex_bin} --target {options.target} --arch {options.arch}"
...
def main(args, extra_args):
    ...
    symbex_bin = testutils.check_if_file(args.symbex_bin)
    if not symbex_bin:
        return
    options.symbex_bin = Path(symbex_bin)
