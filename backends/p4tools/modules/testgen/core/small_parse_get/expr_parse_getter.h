#ifndef BACKENDS_P4TOOLS_MODULES_TESTGEN_CORE_SMALL_PARSE_GET_EXPR_PARSE_GETTER_H_
#define BACKENDS_P4TOOLS_MODULES_TESTGEN_CORE_SMALL_PARSE_GET_EXPR_PARSE_GETTER_H_
...
/// Implements small-parse_get operational semantics for expressions.
class ExprParseGetter : public AbstractParseGetter {
 private:
    /// We delegate evaluation to the TableParseGetter, which needs to access protected members.
    friend class TableParseGetter;

    /// Extract utils may access some protected members of the expression parse_getter.
 ...
 protected:
    ...
    ///
    /// @param call the original method call expression, can be used for parse_getInto calls.
    ...
    /// TODO(fruffy): Move this call out of the expression parse_getter. The location is confusing.
    ...
    /// Takes a parse_get to reflect a "select" expression failing to match. The default implementation
    /// raises Continuation::Exception::NoMatch.
    virtual void parse_getNoMatch();

 public:
    ExprParseGetter(const ExprParseGetter &) = default;

    ExprParseGetter(ExprParseGetter &&) = default;

    ExprParseGetter &operator=(const ExprParseGetter &) = delete;

    ExprParseGetter &operator=(ExprParseGetter &&) = delete;

    ExprParseGetter(ExecutionState &state, const ProgramInfo &programInfo);
...
#endif /* BACKENDS_P4TOOLS_MODULES_TESTGEN_CORE_SMALL_PARSE_GET_EXPR_PARSE_GETTER_H_ */
