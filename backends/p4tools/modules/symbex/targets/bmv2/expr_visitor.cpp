...
#include "backends/p4tools/modules/symbex/targets/bmv2/table_visitor.h"
...
void Bmv2V1ModelExprVisitor::resetPreservingFieldList(ExecutionState &nextState,
...
void Bmv2V1ModelExprVisitor::processClone(const ExecutionState &state,
    ...
    // tests?
    // TODO: Circle back to this once we have modifiable state in extern visitors.
    ...
    if (cloneType == BMv2Constants::CloneType::I2E) {
        ...
        for (size_t paramIdx = 0; paramIdx < params->size(); ++paramIdx) {
            ...
            cloneState->copyIn(SymbexTarget::get(), param, archRef);
        }
        ...
        SYMBEX_UNIMPLEMENTED("Unsupported clone type %1%.", cloneType);
    }
    // Attach the clone specification for test generation.
...
const ExprVisitor::ExternMethodImpls<Bmv2V1ModelExprVisitor>::MethodImpl
    Bmv2V1ModelExprVisitor::ASSERT_ASSUME_EXECUTE =
        [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
            const auto *cond = externInfo.externArguments.at(0)->expression;

            if (!SymbolicEnv::isSymbolicValue(cond)) {
                // Evaluate the condition.
                visitor.stepToSubexpr(
                    cond, visitor.result, visitor.state,
            ...
            auto upCasename = externInfo.methodName.name.toUpper();
            ...
            // Handle the case where the condition is true.
            {
                ...
                visitor.result->emplace_back(cond, visitor.state, nextState);
            }
            // Handle the case where the condition is false.
            {
                auto &falseState = visitor.state.clone();
                ...
                visitor.result->emplace_back(new IR::LNot(IR::Type::Boolean::get(), cond),
                                             visitor.state, falseState);
            }
        };

const ExprVisitor::ExternMethodImpls<Bmv2V1ModelExprVisitor>
    Bmv2V1ModelExprVisitor::BMV2_EXTERN_METHOD_IMPLS({
        /* ======================================================================================
         *  mark_to_drop
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             if (resultField->type->is<IR::Type_Bits>()) {
                 nextState.set(fieldRef,
                               visitor.programInfo.createTargetUninitialized(resultField->type, true));
                 nextState.popBody();
                 visitor.result->emplace_back(nextState);
                 return;
             }
         ...
         * ======================================================================================
         */
        {"*method.assume"_cs, {"check"_cs}, ASSERT_ASSUME_EXECUTE},
        /* ======================================================================================
         *  assert
         ...
         * ======================================================================================
         */
        {"*method.assert"_cs, {"check"_cs}, ASSERT_ASSUME_EXECUTE},
        /* ======================================================================================
         *  log_msg
        ...
        {"*method.log_msg"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             auto msg =
                 externInfo.externArguments.at(0)->expression->checkedTo<IR::StringLiteral>()->value;
        ...
        {"*method.hash"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             for (size_t idx = 1; idx < externInfo.externArguments.size(); ++idx) {
                 ...
                 // TODO: Frontload this in the expression visitor for method call expressions.
                 if (!SymbolicEnv::isSymbolicValue(argExpr)) {
                     // Evaluate the condition.
                     visitor.stepToSubexpr(
                         argExpr, visitor.result, visitor.state,
             ...
             const auto *declInstance =
                 visitor.state.findDecl(new IR::PathExpression(externInfo.methodName));
             ...
             auto externName = externInfo.externObjectRef.toString() + "_" +
                               declInstance->controlPlaneName();
             auto &nextState = visitor.state.clone();
             if (hashOutput->type->is<IR::Type_Bits>()) {
                 const auto &fieldRef = ToolsVariables::convertReference(hashOutput);
                 if (argsAreTainted) {
                     nextState.set(fieldRef,
                                   visitor.programInfo.createTargetUninitialized(fieldRef->type, false));
                 } else {
                 ...
                 SYMBEX_UNIMPLEMENTED("Hash output %1% of type %2% not supported", hashOutput,
                                       hashOutput->type);
             }
        ...
        {"register.read"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             const auto *externRef = externInfo.externObjectRef.checkedTo<IR::PathExpression>();
             const auto &externInstance = visitor.state.findDecl(externRef);
             ...
             const auto *registerState = visitor.state.getTestObject(
             ...
             if (registerState != nullptr) {
                 ...
                 const auto *inputValue =
                     visitor.programInfo.createTargetUninitialized(readOutput->type, false);
             ...
             if (readOutput->type->is<IR::Type_Bits>()) {
                 ...
                 SYMBEX_UNIMPLEMENTED("Read extern output %1% of type %2% not supported",
        ...
        {"register.write"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             const auto &externInstance = visitor.state.findDecl(receiverPath);
             auto &nextState = visitor.state.clone();
             // TODO: Find a better way to model a trace of this event.
             std::stringstream registerStream;
             ...
             if (registerState != nullptr) {
                 ...
                 const auto &writeValue =
                     visitor.programInfo.createTargetUninitialized(inputValue->type, false);
        ...
        {"meter.execute_meter"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             auto testBackend = SymbexOptions::get().testBackend;
             ...
             if (!SymbolicEnv::isSymbolicValue(index)) {
                 // Evaluate the condition.
                 visitor.stepToSubexpr(
                     index, visitor.result, visitor.state,
                     [&externInfo](const Continuation::Parameter *v) {
                         auto *clonedCall = externInfo.originalCall.clone();
                         auto *arguments = clonedCall->arguments->clone();
             ...
             const auto *meterState = visitor.state.getTestObject(
                 "meter_values"_cs, externInstance->controlPlaneName(), false);
             Bmv2V1ModelMeterValue *meterValue = nullptr;
             const auto &inputValue = ToolsVariables::getSymbolicVariable(
                 meterResult->type,
                 "meter_value" + std::to_string(externInfo.originalCall.clone_id));
        ...
        {"direct_meter.read"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             const auto *progInfo = visitor.getProgramInfo().checkedTo<Bmv2V1ModelProgramInfo>();
             const auto *receiverPath = externInfo.externObjectRef.checkedTo<IR::PathExpression>();
             const auto *externInstance = visitor.state.findDecl(receiverPath);
             const auto *table = progInfo->getTableofDirectExtern(externInstance);
             const auto *tableEntry =
                 visitor.state.getTestObject("tableconfigs"_cs, table->controlPlaneName(), false);
        ...
        {"*method.clone_preserving_field_list"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             if (visitor.state.hasProperty("recirculate_count"_cs)) {
                 if (visitor.state.getProperty<uint64_t>("recirculate_count"_cs) > 0) {
                     auto &nextState = visitor.state.clone();
             ...
             auto cloneType =
                 externInfo.externArguments.at(0)->expression->checkedTo<IR::Constant>()->asUint64();
             ...
             auto *cloneInfo = new Bmv2V1ModelCloneInfo(
                 sessionIdExpr, static_cast<BMv2Constants::CloneType>(cloneType),
                 visitor.state.clone(), preserveIndex);
             auto &nextState = visitor.state.clone();
        ...
        {"*method.resubmit_preserving_field_list"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             // TODO: Determine the exact count.
             if (visitor.state.hasProperty("recirculate_count"_cs)) {
                 recirculateCount = visitor.state.getProperty<uint64_t>("recirculate_count"_cs);
        ...
        {"*method.clone"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             auto *cloneInfo = new Bmv2V1ModelCloneInfo(
                 sessionIdExpr, static_cast<BMv2Constants::CloneType>(cloneType),
                 visitor.state.clone(), std::nullopt);
             nextState.addTestObject("clone_infos"_cs, "clone_info"_cs, cloneInfo);
             // Also set clone as active, which will trigger "processClone" in the deparser.
             nextState.setProperty("clone_active"_cs, true);
             nextState.popBody();
        ...
        {"*.invoke_traffic_manager"_cs,
         ...
         [](const ExternInfo & /*externInfo*/, Bmv2V1ModelExprVisitor &visitor) {
             ...
             if (visitor.state.hasProperty("clone_active"_cs) &&
                 visitor.state.getProperty<bool>("clone_active"_cs)) {
                 visitor.processClone(visitor.state, visitor.result);
                 return;
             }

             // Check whether recirculate is active.
             if (visitor.state.hasProperty("recirculate_active"_cs) &&
                 visitor.state.getProperty<bool>("recirculate_active"_cs)) {
                 visitor.processRecirculate(visitor.state, visitor.result);
                 return;
             }
        ...
        {"*method.verify_checksum"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             auto algo = Bmv2HashAlgorithm(
                 externInfo.externArguments.at(3)->expression->checkedTo<IR::Constant>()->asInt());
             ...
             if (const auto *boolVal = verifyCond->to<IR::BoolLiteral>()) {
                 if (!boolVal->value) {
                     auto &taintedState = visitor.state.clone();
                     taintedState.popBody();
                     visitor.result->emplace_back(taintedState);
                     return;
                 }
             }
             ...
             if (argsAreTainted) {
                 ...
                 taintedState.set(checksumErr,
                                  visitor.programInfo.createTargetUninitialized(checksumErr->type, true));
                 taintedState.popBody();
                 visitor.result->emplace_back(taintedState);
                 return;
             }
             ...
             for (size_t idx = 0; idx < exprList.size(); ++idx) {
                 ...
                 if (!expr->is<IR::Constant>()) {
                     auto &rejectState = visitor.state.clone();
                     ...
                     visitor.result->emplace_back(rejectState);
                     return;
                 }
             }
             ...
             computedResult = visitor.computeChecksum(exprList, algo);
             ...
             // The condition is true and the checksum matches.
             {
                 ...
                 nextState.popBody();
                 visitor.result->emplace_back(new IR::LAnd(checksumMatchCond, verifyCond),
             ...
             // The condition is true and the checksum does not match.
             {
                 ...
                 nextState.replaceTopBody(&replacements);
                 visitor.result->emplace_back(errorCond, visitor.state, nextState);
             }
             ...
             // Handle the case where the condition is false.
             {
                 auto &nextState = visitor.state.clone();
                 nextState.popBody();
                 visitor.result->emplace_back(new IR::LNot(IR::Type::Boolean::get(), verifyCond),
                                              visitor.state, nextState);
             }
         }},
        /* ======================================================================================
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             if (argsAreTainted) {
                 ...
                 taintedState.set(checksumVar,
                                  visitor.programInfo.createTargetUninitialized(checksumVarType, true));
                 taintedState.popBody();
                 visitor.result->emplace_back(taintedState);
                 return;
             }
             ...
             // Handle the case where the condition is true.
             {
                 ...
                 nextState.popBody();
                 visitor.result->emplace_back(updateCond, visitor.state, nextState);
             }
             // Handle the case where the condition is false. No change here.
             {
                 auto &nextState = visitor.state.clone();
                 nextState.popBody();
                 visitor.result->emplace_back(new IR::LNot(IR::Type::Boolean::get(), updateCond),
                                              visitor.state, nextState);
             }
         }},
        /* ======================================================================================
        ...
        {"*method.update_checksum_with_payload"_cs,
         ...
         [](const ExternInfo &externInfo, Bmv2V1ModelExprVisitor &visitor) {
             ...
             const auto &checksumVar =
                 ToolsVariables::convertReference(externInfo.externArguments.at(2)->expression);
             const auto *updateCond = externInfo.externArguments.at(0)->expression;
             const auto *checksumVarType = checksumVar->type;
             const auto *data = externInfo.externArguments.at(1)->expression;
             auto algo = Bmv2HashAlgorithm(
...
void Bmv2V1ModelExprVisitor::evalExternMethodCall(const ExternInfo &externInfo) {
    ...
    return ExprVisitor::evalExternMethodCall(externInfo);
}

bool Bmv2V1ModelExprVisitor::preorder(const IR::P4Table *table) {
    // Delegate to the tableVisitor.
    Bmv2V1ModelTableVisitor tableVisitor(this, table, testCase);
