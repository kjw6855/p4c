...
class BNGTest(P4EbpfTest):
    ...
    def setup_port(
        self, port_id, vlan_id, port_type, double_tagged=False, inner_vlan_id=0
    ):
        if double_tagged:
    ...
    def set_egress_vlan(self, egress_port, vlan_id, push_vlan=False):
        ...
        self.table_add(
            table="egress_egress_vlan", key=[vlan_id, egress_port], action=action_id
        )
    ...
    def set_forwarding_type(
        ...
        if ethertype == ETH_TYPE_IPV4:
            ...
            key_ip_eth_type = ETH_TYPE_IPV4
        key_eth_dst = (
            "{}^0xffffffffffff".format(eth_dstAddr)
    ...
    def add_forwarding_routing_v4_entry(
        self, ipv4_dstAddr, ipv4_pLen, egress_port, smac, dmac
    ):
        self.table_add(
    ...
    def add_next_vlan(self, port_id, new_vlan_id):
        self.table_add(
            table="ingress_next_vlan", key=[port_id], action=1, data=[new_vlan_id]
        )
    ...
    def set_line_map(self, s_tag, c_tag, line_id):
        assert line_id != 0
        self.table_add(
            table="ingress_t_line_map", key=[s_tag, c_tag], action=1, data=[line_id]
        )

    def setup_line_v4(
        self, s_tag, c_tag, line_id, ipv4_addr, pppoe_session_id, enabled=True
    ):
        assert s_tag != 0
        assert c_tag != 0
        assert line_id != 0
...
class PPPoEUpstreamTest(BNGTest):
    ...
    def runTest(self):
        ...
        self.add_forwarding_routing_v4_entry(
            SERVER_IP, 24, DP_PORTS[1], SWITCH_MAC, HOST1_MAC
        )
...
class PPPoEDownstreamTest(BNGTest):
    ...
    def runTest(self):
        ...
        self.add_forwarding_routing_v4_entry(
            CLIENT_IP, 24, DP_PORTS[0], SWITCH_MAC, HOST1_MAC
        )
