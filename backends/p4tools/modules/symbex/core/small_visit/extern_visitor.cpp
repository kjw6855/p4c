...
#include "backends/p4tools/modules/symbex/core/small_visit/expr_visitor.h"  // IWYU pragma: associated
...
std::vector<std::pair<IR::StateVariable, const IR::Expression *>> ExprVisitor::setFields(
...
ExprVisitor::PacketCursorAdvanceInfo ExprVisitor::calculateSuccessfulParserAdvance(
    ...
    auto *cond = new IR::Geq(IR::Type::Boolean::get(),
                             IR::Constant::get(&PacketVars::PACKET_SIZE_VAR_TYPE, remainderSize),
                             IR::Constant::get(&PacketVars::PACKET_SIZE_VAR_TYPE, advanceSize));
    return {advanceSize, cond, advanceSize, new IR::LNot(cond)};
}

ExprVisitor::PacketCursorAdvanceInfo ExprVisitor::calculateAdvanceExpression(
...
const ExprVisitor::ExternMethodImpls<ExprVisitor> ExprVisitor::INTERNAL_EXTERN_METHOD_IMPLS({
    /* ======================================================================================
    ...
    {"*.prepend_to_prog_header"_cs,
        {"hdr"_cs},
        [](const ExternInfo &externInfo, ExprVisitor &visitor) {
            const auto *prependVar = externInfo.externArguments.at(0)->expression;
            auto &nextState = visitor.state.clone();
            const auto *prependType = visitor.state.resolveType(prependVar->type);
            ...
            if (const auto *structExpr = prependVar->to<IR::StructExpression>()) {
                ...
                SYMBEX_UNIMPLEMENTED("Prepend input %1% of type %2% not supported", prependVar,
                                    prependType);
            }
            nextState.popBody();
    ...
    {"*.append_to_prog_header"_cs,
        ...
        [](const ExternInfo &externInfo, ExprVisitor &visitor) {
            ...
            auto &nextState = visitor.state.clone();
            const auto *appendType = visitor.state.resolveType(appendVar->type);
            ...
            if (const auto *structExpr = appendVar->to<IR::StructExpression>()) {
                ...
                SYMBEX_UNIMPLEMENTED("Append input %1% of type %2% not supported", appendVar,
                                    appendType);
            }
            nextState.popBody();
            visitor.result->emplace_back(nextState);
    ...
    {"*.prepend_emit_buffer"_cs,
        ...
        [](const ExternInfo & /*externInfo*/, ExprVisitor &visitor) {
            auto &nextState = visitor.state.clone();
            const auto *emitBuffer = visitor.state.getEmitBuffer();
    ...
    {"*.drop_and_exit"_cs,
        ...
        [](const ExternInfo & /*externInfo*/, ExprVisitor &visitor) {
            auto &nextState = visitor.state.clone();
            const auto *drop =
                visitor.state.getSymbolicEnv().subst(visitor.programInfo.dropIsActive());
            // If the drop variable is tainted, we also mark the port tainted.
            if (Taint::hasTaint(drop)) {
                nextState.set(visitor.programInfo.getTargetOutputPortVar(),
                              visitor.programInfo.createTargetUninitialized(
                                  visitor.programInfo.getTargetOutputPortVar()->type, true));
            }
    ...
    {"*.copy_in"_cs,
        ...
        [](const ExternInfo &externInfo, ExprVisitor &visitor) {
            ...
            const auto *block = visitor.state.findDecl(new IR::Path(blockRef->value));
            auto blockName = block->getName().name;
            auto &nextState = visitor.state.clone();
            auto canonicalName = visitor.programInfo.getCanonicalBlockName(blockName);
            const auto &archSpec = visitor.programInfo.getArchSpec();
            ...
            for (size_t paramIdx = 0; paramIdx < blockParams->size(); ++paramIdx) {
                ...
                nextState.copyIn(SymbexTarget::get(), internalParam, externalParamName);
            }
...
void ExprVisitor::evalInternalExternMethodCall(const ExternInfo &externInfo) {
...
const ExprVisitor::ExternMethodImpls<ExprVisitor> ExprVisitor::CORE_EXTERN_METHOD_IMPLS({
    /* ======================================================================================
     *  packet_in.lookahead
    ...
    {"packet_in.lookahead"_cs,
     ...
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         ...
         auto condInfo =
             visitor.calculateSuccessfulParserAdvance(visitor.state, lookaheadType->width_bits());
         ...
         if (condInfo.advanceCond != nullptr) {
             ...
             visitor.result->emplace_back(condInfo.advanceCond, visitor.state, nextState);
         }
         ...
         if (condInfo.advanceFailCond != nullptr) {
             ...
             visitor.result->emplace_back(condInfo.advanceFailCond, visitor.state, rejectState);
         }
     }},
    /* ======================================================================================
     *  packet_in.advance
    ...
    {"packet_in.advance"_cs,
     ...
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         ...
         if (!SymbolicEnv::isSymbolicValue(advanceExpr)) {
             visitor.stepToSubexpr(advanceExpr, visitor.result, visitor.state,
         ...
         if (const auto *advanceConst = advanceExpr->to<IR::Constant>()) {
             ...
             condInfo = visitor.calculateSuccessfulParserAdvance(visitor.state, advanceValue);
         } else {
             ...
             condInfo =
                 visitor.calculateAdvanceExpression(visitor.state, advanceExpr, restrictions);
         }
    ...
    {"packet_in.extract"_cs,
     ...
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         ...
         const auto *initialType = visitor.state.resolveType(typeArgs->at(0));
         const auto *extractedType = initialType->checkedTo<IR::Type_StructLike>();

         // Calculate the conditions for a failed or successful advance, given the size.
         auto extractSize = extractedType->width_bits();
         auto condInfo = visitor.calculateSuccessfulParserAdvance(visitor.state, extractSize);
         ...
         if (condInfo.advanceCond != nullptr) {
             ...
             if (extractedType->is<IR::Type_Header>()) {
                 visitor.setHeaderValidity(extractOutput, true, nextState);
             }
             // Add an event signifying that this extract was successful.
             // It also records the condition we are passing.
             auto pktCursor = visitor.state.getInputPacketCursor();
         ...
         if (condInfo.advanceFailCond != nullptr) {
             ...
             rejectState.add(*new TraceEvents::ExtractFailure(
                 extractOutput, visitor.state.getInputPacketCursor(), condInfo.advanceFailCond));
             rejectState.replaceTopBody(Continuation::Exception::PacketTooShort);
    ...
    {"packet_in.extract"_cs,
     ...
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         ...
         if (!SymbolicEnv::isSymbolicValue(varbitExtractExpr)) {
             visitor.stepToSubexpr(varbitExtractExpr, visitor.result, visitor.state,
                                   [&externInfo](const Continuation::Parameter *v) {
                                       auto *clonedCall = externInfo.originalCall.clone();
                                       auto *arguments = clonedCall->arguments->clone();
         ...
         if (const auto *varbitConst = varbitExtractExpr->to<IR::Constant>()) {
             ...
             condInfo = visitor.calculateSuccessfulParserAdvance(visitor.state,
             ...
             condInfo =
                 visitor.calculateAdvanceExpression(visitor.state, varbitExtractExpr, restrictions);
         ...
         if (condInfo.advanceCond != nullptr) {
             ...
             if (varbit->size < varBitFieldSize) {
                 auto &nextState = visitor.state.clone();
                 nextState.set(visitor.state.getCurrentParserErrorLabel(),
                               IR::Constant::get(visitor.programInfo.getParserErrorType(),
             ...
             nextState.add(*new TraceEvents::ExtractSuccess(
                 extractOutput, visitor.state.getInputPacketCursor(), condInfo.advanceCond,
                 fields));
             nextState.popBody();
             visitor.result->emplace_back(condInfo.advanceCond, visitor.state, nextState);
         }
    ...
    {"packet_in.length"_cs,
     ...
     [](const ExternInfo & /*externInfo*/, ExprVisitor &visitor) {
         ...
         visitor.result->emplace_back(std::nullopt, visitor.state, nextState);
     }},
    /* ======================================================================================
     *  packet_out.emit
    ...
    {"packet_out.emit"_cs,
     ...
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         ...
         // unravels emit calls on structs into emit calls on the header members.
         {
             ...
             // delta is adjusted.
             visitor.result->emplace_back(validVar, visitor.state, nextState);
         }
         {
             auto &invalidState = visitor.state.clone();
             ...
             visitor.result->emplace_back(new IR::LNot(IR::Type::Boolean::get(), validVar),
                                          visitor.state, invalidState);
         }
     }},
    /* ======================================================================================
     *  verify
    ...
    {"*method.verify"_cs,
     ...
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         ...
         if (!SymbolicEnv::isSymbolicValue(cond)) {
             ...
             visitor.stepToSubexpr(cond, visitor.result, visitor.state,
                                   [&externInfo](const Continuation::Parameter *v) {
                                       auto *clonedCall = externInfo.originalCall.clone();
         ...
         if (Taint::hasTaint(cond)) {
             ...
             const auto &errVar = visitor.state.getCurrentParserErrorLabel();
         ...
         const auto &errVar = visitor.state.getCurrentParserErrorLabel();
         falseState.set(errVar, IR::Constant::get(visitor.programInfo.getParserErrorType(),
                                                   error->value));
         falseState.replaceTopBody(Continuation::Exception::Reject);
         visitor.result->emplace_back(new IR::LNot(IR::Type::Boolean::get(), cond), visitor.state,
                                      falseState);
     }},
    /* ======================================================================================
     *  assume
     * ====================================================================================== */
    {"*method.symbex_assume"_cs,
     {"check"_cs},
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         // Do nothing if assumption mode is not active.
         if (!SymbexOptions::get().enforceAssumptions) {
             auto &nextState = visitor.state.clone();
             nextState.popBody();
             visitor.result->emplace_back(nextState);
             return;
     ...
     *  assert
     * ====================================================================================== */
    {"*method.symbex_assert"_cs,
     {"check"_cs},
     [](const ExternInfo &externInfo, ExprVisitor &visitor) {
         // Do nothing if assumption or assertion mode is not active.
         auto enforceAssumptions = SymbexOptions::get().enforceAssumptions;
         auto assertionModeEnabled = SymbexOptions::get().assertionModeEnabled;
         ...
         if (assertionModeEnabled) {
             ...
             visitor.result->emplace_back(new IR::LNot(cond), visitor.state, nextState);
         }

         auto &passState = visitor.state.clone();
         passState.popBody();
         // If enforceAssumptions is active. Add the condition to the path conditions.
         if (enforceAssumptions) {
             visitor.result->emplace_back(cond, visitor.state, passState);
         } else {
             visitor.result->emplace_back(passState);
         }
     }},
});

void ExprVisitor::evalExternMethodCall(const ExternInfo &externInfo) {
