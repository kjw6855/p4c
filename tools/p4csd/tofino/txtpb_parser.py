...
The grammar follows ``targets/tofino/proto/p4symbex_bfrt.proto``:
  ...
  affected_register {
    ...
    hit_phase: 1                    # optional
    miss_phase: 3                   # optional
  }
...
def parse_tampering_txtpb(path: Path) -> TamperCase:
    ...
    if current is not None:
        phases.append(current)
    # Pad to three phases (malformed files get padded with empty trailing phases that
    # the harness will detect downstream).
    while len(phases) < 3:
        phases.append(Phase())
    return TamperCase(phases=phases[:3], entities=entities,
