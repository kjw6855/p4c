...
#include "backends/p4tools/modules/symbex/targets/tofino/rename_keys.h"
...
/* =============================================================================================
 *  AbstractTofinoSymbexTarget implementation
 * ============================================================================================= */

AbstractTofinoSymbexTarget::AbstractTofinoSymbexTarget(const std::string &deviceName,
                                                         const std::string &archName)
    : SymbexTarget(deviceName, archName) {}

TofinoTestBackend *AbstractTofinoSymbexTarget::getTestBackendImpl(
...
P4::FrontEnd AbstractTofinoSymbexTarget::mkFrontEnd() const {
    // FIXME: Reenable this.
    // return P4::FrontEnd(BFN::ParseAnnotations());
    return {};
}

MidEnd AbstractTofinoSymbexTarget::mkMidEnd(const CompilerOptions &options) const {
...
CompilerResultOrError AbstractTofinoSymbexTarget::runCompilerImpl(
...
/* =============================================================================================
 *  Tofino_TnaSymbexTarget implementation
 * ============================================================================================= */

Tofino_TnaSymbexTarget::Tofino_TnaSymbexTarget() : AbstractTofinoSymbexTarget("tofino", "tna") {}

void Tofino_TnaSymbexTarget::make() {
    static Tofino_TnaSymbexTarget *INSTANCE = nullptr;
    if (INSTANCE == nullptr) {
        INSTANCE = new Tofino_TnaSymbexTarget();
    }
}

const TofinoProgramInfo *Tofino_TnaSymbexTarget::produceProgramInfoImpl(
...
TofinoCmdStepper *Tofino_TnaSymbexTarget::getCmdStepperImpl(ExecutionState &state,
                                                             AbstractSolver &solver,
                                                             const ProgramInfo &programInfo) const {
    return new TofinoCmdStepper(state, solver, programInfo);
}

Tofino1ExprStepper *Tofino_TnaSymbexTarget::getExprStepperImpl(
...
JBay_T2naSymbexTarget::JBay_T2naSymbexTarget() : AbstractTofinoSymbexTarget("tofino2", "t2na") {}

void JBay_T2naSymbexTarget::make() {
    static JBay_T2naSymbexTarget *INSTANCE = nullptr;
    if (INSTANCE == nullptr) {
        INSTANCE = new JBay_T2naSymbexTarget();
    }
}

const JBayProgramInfo *JBay_T2naSymbexTarget::produceProgramInfoImpl(
    const CompilerResult &compilerResult, const IR::Declaration_Instance *mainDecl) const {
    // Ensure that main instantiates Switch.
    // TODO: Handle MultiParserSwitch.
    const auto *mainType = mainDecl->type->to<IR::Type_Specialized>();
...
JBayCmdStepper *JBay_T2naSymbexTarget::getCmdStepperImpl(ExecutionState &state,
                                                          AbstractSolver &solver,
                                                          const ProgramInfo &programInfo) const {
    return new JBayCmdStepper(state, solver, programInfo);
}

JBayExprStepper *JBay_T2naSymbexTarget::getExprStepperImpl(ExecutionState &state,
                                                            AbstractSolver &solver,
                                                            const ProgramInfo &programInfo) const {
