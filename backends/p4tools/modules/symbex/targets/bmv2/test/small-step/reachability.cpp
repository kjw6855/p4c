...
TEST_F(P4CReachability, testTableAndActions) {
    auto result = loadExampleForReachability(
        "backends/p4tools/modules/symbex/targets/bmv2/test/p4-programs/bmv2_hit.p4");
...
TEST_F(P4CReachability, testIfStatement) {
    ...
    auto result = loadExampleForReachability(
        "backends/p4tools/modules/symbex/targets/bmv2/test/p4-programs/bmv2_if.p4");
    const auto *program = get<0>(result);
    ASSERT_TRUE(program);
    const auto *dcg = std::get<1>(result);
    ASSERT_TRUE(dcg);
...
void callSymbex(const char *inputFile, const char *behavior, const char *path, int maxTests) {
    ...
    std::ostringstream cmdSymbex;
    cmdSymbex << buildPath << "symbex ";
    cmdSymbex << "-I \"" << buildPath << "p4include\" --target bmv2  --std p4-16 ";
    cmdSymbex << "--test-backend STF --arch v1model --seed 1000 --max-tests " << maxTests << "  ";
    cmdSymbex << "--pattern \"" << behavior << "\" ";
    cmdSymbex << "--out-dir \"" << buildPath << path << "\" \"" << sourcePath << prefix;
    cmdSymbex << inputFile << "\"";
    if (system(cmdSymbex.str().c_str()) != 0) {
        BUG("symbex failed to run - %1%", cmdSymbex.str());
    }
}
...
TEST_F(P4CReachability, testReachabilityEngineActions) {
    callSymbex("backends/p4tools/modules/symbex/targets/bmv2/test/p4-programs/bmv2_miss.p4",
...
TEST_F(P4CReachability, testReachabilityEngineTables) {
    callSymbex("backends/p4tools/modules/symbex/targets/bmv2/test/p4-programs/bmv2_action_run.p4",
                "ingress.MyAction2", "tmp", 10);
    std::list<std::list<std::string>> ids = {{" MyAction2"}};
    ASSERT_TRUE(checkResultingSTF(ids, "tmp"));
}
...
TEST_F(P4CReachability, testReachabilityEngineTable2) {
    callSymbex("backends/p4tools/modules/symbex/targets/bmv2/test/p4-programs/bmv2_if.p4",
