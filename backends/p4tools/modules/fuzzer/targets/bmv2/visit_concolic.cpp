...
std::vector<char> Bmv2VisitConcolic::convertBigIntToBytes(big_int& dataInt, int targetWidthBits) {
...
const IR::Expression* Bmv2VisitConcolic::setAndComputePayload(
    const Model& completedModel, VisitConcolicVariableMap* resolvedVisitConcolicVariables, int payloadSize) {
    const auto* payloadType = IR::getBitType(payloadSize);
    const auto& payLoadVar = StateVariable(VisitState::getPayloadLabel(payloadType));
    const auto* payloadExpr = completedModel.get(payLoadVar, false);
    // If the variable already has been fixed, return it
    auto it = resolvedVisitConcolicVariables->find(payLoadVar);
    if (it != resolvedVisitConcolicVariables->end()) {
        return it->second;
    }
    ...
    resolvedVisitConcolicVariables->emplace(payLoadVar, payloadExpr);
    return payloadExpr;
}

big_int Bmv2VisitConcolic::computeChecksum(const std::vector<const IR::Expression*>& exprList,
...
const VisitConcolicMethodImpls::ImplList Bmv2VisitConcolic::Bmv2VisitConcolicMethodImpls{
    /* ======================================================================================
     *   method_hash
     * ====================================================================================== */
     ...
     [](cstring /*concolicMethodName*/, const IR::ConcolicVariable* var,
        ...
        VisitConcolicVariableMap* resolvedVisitConcolicVariables) {
         ...
         if (const auto* checksumVarType = checksumVar->type->to<IR::Type_Bits>()) {
             ...
             (*resolvedVisitConcolicVariables)[concolicMember] =
         ...
         // We can not use resolvedVisitConcolic variables here because there might be multiple resolved
         ...
         for (const auto& variable : resolvedExpressions) {
             ...
             (*resolvedVisitConcolicVariables)[varName] = varExpr;
         }
     }},
    /* ======================================================================================
     *   method_checksum
    ...
    {"*method_checksum_with_payload",
     ...
     [](cstring /*concolicMethodName*/, const IR::ConcolicVariable* var,
        ...
        VisitConcolicVariableMap* resolvedVisitConcolicVariables) {
         ...
         // This is the maximum value this checksum can have.
         auto maxHashInt = IR::getMaxBvVal(checksumVarType);
         const auto& packetBitSizeVar = VisitState::getInputPacketSizeVar();
         ...
         if (payloadSize > 0) {
             const auto* payloadExpr =
                 setAndComputePayload(*completedModel, resolvedVisitConcolicVariables, payloadSize);
             // Fix the payload size only if is not fixed already.
             resolvedVisitConcolicVariables->emplace(packetBitSizeVar, payloadSizeConst);
...
const VisitConcolicMethodImpls::ImplList* Bmv2VisitConcolic::getBmv2VisitConcolicMethodImpls() {
    return &Bmv2VisitConcolicMethodImpls;
}
