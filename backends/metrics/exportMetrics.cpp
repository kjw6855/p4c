...
bool ExportMetricsPass::preorder(const IR::P4Program * /*program*/) {
    ...
    pos = isolatedFileName.find_last_of("/\\");
    if (pos != std::string::npos)
        isolatedFileName = isolatedFileName.substr(pos + 1);

    if (!std::filesystem::exists(dirname)) {
        std::error_code ec;
        if (!std::filesystem::create_directories(dirname, ec)) {
            error(ErrorType::ERR_IO, "Error: Unable to create directory %s: %s",
                  dirname.string().c_str(), ec.message().c_str());
            return false;
        }
    }

    // Use dirname and base of filename to construct full paths for output files.
    std::string prefixPath = dirname.string() + "/" + isolatedFileName;

    std::ofstream textFile(prefixPath + ".txt");
    if (!textFile.is_open()) {
        error(ErrorType::ERR_IO, "Error: Unable to open file %s.txt", isolatedFileName.c_str());
        return false;
    }

    std::ofstream jsonFile(prefixPath + ".json");
    if (!jsonFile.is_open()) {
