...
#include "backends/p4tools/modules/testgen/targets/tna/backend/metadata/metadata.h"
#include "backends/p4tools/modules/testgen/targets/tna/backend/protobuf/protobuf.h"
#include "backends/p4tools/modules/testgen/targets/tna/backend/ptf/ptf.h"
#include "backends/p4tools/modules/testgen/targets/tna/backend/stf/stf.h"
...
const big_int TnaTestBackend::ZERO_PKT_VAL = 0x2000000;
const big_int TnaTestBackend::ZERO_PKT_MAX = 0xffffffff;
const std::set<std::string> TnaTestBackend::SUPPORTED_BACKENDS = {"PTF", "STF", "PROTOBUF",
                                                                   "METADATA"};

TnaTestBackend::TnaTestBackend(const ProgramInfo &programInfo, SymbolicExecutor &symbex,
...
TestBackEnd::TestInfo TnaTestBackend::produceTestInfo(
    ...
    if (testInfo.outputPacket->type->width_bits() == 0) {
        ...
        testInfo.outputPacket =
            IR::getConstant(IR::getBitType(outPktSize), TnaTestBackend::ZERO_PKT_VAL);
        testInfo.packetTaintMask =
            IR::getConstant(IR::getBitType(outPktSize), TnaTestBackend::ZERO_PKT_MAX);
    }
    return testInfo;
}

const TestSpec *TnaTestBackend::createTestSpec(const ExecutionState *executionState,
    ...
    if (TestgenOptions::get().testBackend == "METADATA") {
        ...
        const auto *tnaProgInfo = programInfo.checkedTo<TnaProgramInfo>();
        const auto *localMetadataVar = tnaProgInfo->getBlockParam("Parser", 2);
    ...
    for (const auto &testObject : actionProfiles) {
        ...
        const auto *actionProfile = testObject.second->checkedTo<TnaActionProfile>();
    ...
    for (const auto &testObject : actionSelectors) {
        ...
        const auto *actionSelector = testObject.second->checkedTo<TnaActionSelector>();
    ...
    for (const auto &testObject : cloneSpecs) {
        ...
        const auto *cloneSpec = testObject.second->checkedTo<TnaCloneSpec>();
    ...
    for (const auto &testObject : meterInfos) {
        ...
        const auto *meterInfo = testObject.second->checkedTo<TnaMeterValue>();
