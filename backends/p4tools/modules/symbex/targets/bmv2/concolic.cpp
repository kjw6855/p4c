...
big_int Bmv2Concolic::checksum(Bmv2HashAlgorithm algo, const uint8_t *buf, size_t len) {
    ...
    switch (algo) {
        ...
        default:
            SYMBEX_UNIMPLEMENTED("Algorithm %1% not implemented for hash.", algo);
    }
}
     ...
     [](cstring /*concolicMethodName*/, const IR::ConcolicVariable *var,
        ...
        ConcolicVariableMap *resolvedConcolicVariables) {
         ...
         if (!(checksumVar->is<IR::Member>() || checksumVar->is<IR::PathExpression>())) {
             SYMBEX_UNIMPLEMENTED("Checksum input %1% of type %2% not supported", checksumVar,
         ...
         if (exprList.empty()) {
             SYMBEX_UNIMPLEMENTED("Data input is empty. This case is not implemented.");
         }
         ...
         if (const auto *checksumVarType = checksumVar->type->to<IR::Type_Bits>()) {
             ...
             SYMBEX_UNIMPLEMENTED("Checksum output %1% of type %2% not supported", checksumVar,
                                   checksumVar->type);
         }
