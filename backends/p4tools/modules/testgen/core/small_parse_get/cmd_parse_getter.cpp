...
CmdParseGetter::CmdParseGetter(ExecutionState &state,
                       const ProgramInfo &programInfo)
    : AbstractParseGetter(state, programInfo) {}

bool CmdParseGetter::preorder(const IR::AssignmentStatement *assign) {
    logParseGet(assign);

    if (!SymbolicEnv::isSymbolicValue(assign->right)) {
        return parseGetToSubexpr(assign->right, result, state,
...
bool CmdParseGetter::preorder(const IR::P4Parser *p4parser) {
    logParseGet(p4parser);
...
bool CmdParseGetter::preorder(const IR::P4Control *p4control) {
...
bool CmdParseGetter::preorder(const IR::EmptyStatement * /*empty*/) {
    state.popBody();
    result->emplace_back(state);
    return false;
}

bool CmdParseGetter::preorder(const IR::IfStatement *ifStatement) {
    logParseGet(ifStatement);

    if (!SymbolicEnv::isSymbolicValue(ifStatement->condition)) {
        // Evaluate the condition.
        return parseGetToSubexpr(ifStatement->condition, result, state,
...
bool CmdParseGetter::preorder(const IR::MethodCallStatement *methodCallStatement) {
    logParseGet(methodCallStatement);
...
bool CmdParseGetter::preorder(const IR::P4Program * /*program*/) {
    // Don't invoke logParseGet for the top-level program, as that would be overly verbose.
    ...
    std::vector<Continuation::Command> parserBlocks;

    // FIXME: Works for BMv2 parser only
    parserBlocks.push_back((*topLevelBlocks)[0]);
    parserBlocks.push_back((*topLevelBlocks)[1]);
    parserBlocks.push_back((*topLevelBlocks)[2]);
    ...
    // After, we insert the first (parser) program command into the new body and push it to the top.
    // get only first parser
    Continuation::Body newBody(parserBlocks);
    state.replaceBody(newBody);
...
bool CmdParseGetter::preorder(const IR::ParserState *parserState) {
    logParseGet(parserState);
...
/*
bool CmdParseGetter::preorder(const IR::BlockStatement *block) {
    logParseGet(block);
...
bool CmdParseGetter::preorder(const IR::ExitStatement *e) {
    logParseGet(e);
    auto &nextState = state.clone();
...
const Constraint *CmdParseGetter::startParser(const IR::P4Parser *parser, ExecutionState &nextState) {
...
/*
IR::SwitchStatement *CmdParseGetter::replaceSwitchLabels(const IR::SwitchStatement *switchStatement) {
    ...
    CHECK_NULL(table);
    auto actionVar = TableParseGetter::getTableActionVar(table);
...
bool CmdParseGetter::preorder(const IR::SwitchStatement *switchStatement) {
    logParseGet(switchStatement);
