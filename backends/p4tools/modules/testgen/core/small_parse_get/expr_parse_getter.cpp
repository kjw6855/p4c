...
ExprParseGetter::ExprParseGetter(ExecutionState &state,
                         const ProgramInfo &programInfo)
    : AbstractParseGetter(state, programInfo) {}

bool ExprParseGetter::preorder(const IR::BoolLiteral *boolLiteral) {
    logParseGet(boolLiteral);
    return parseGetSymbolicValue(boolLiteral);
}

bool ExprParseGetter::preorder(const IR::Constant *constant) {
    logParseGet(constant);
    return parseGetSymbolicValue(constant);
}

/*
void ExprParseGetter::handleHitMissActionRun(const IR::Member *member) {
    ...
    if (member->member.name == IR::Type_Table::hit) {
        replacements.emplace_back(Continuation::Return(TableParseGetter::getTableHitVar(table)));
    } else if (member->member.name == IR::Type_Table::miss) {
        replacements.emplace_back(
            Continuation::Return(new IR::LNot(TableParseGetter::getTableHitVar(table))));
    } else {
        replacements.emplace_back(Continuation::Return(TableParseGetter::getTableActionVar(table)));
    }
...
bool ExprParseGetter::preorder(const IR::Member *member) {
    logParseGet(member);
    ...
    return parseGetSymbolicValue(state.get(member));
}

bool ExprParseGetter::preorder(const IR::ArrayIndex *arr) { return parseGetSymbolicValue(state.get(arr)); }

void ExprParseGetter::evalActionCall(const IR::P4Action *action, const IR::MethodCallExpression *call) {
...
bool ExprParseGetter::preorder(const IR::MethodCallExpression *call) {
    logParseGet(call);
        ...
        if (const auto *method = call->method->to<IR::Member>()) {
            ...
            if (method->expr->type->is<IR::Type_Header>() ||
                ...
                if (method->member == "isValid") {
                    return parse_getGetHeaderValidity(method->expr);
                }

                if (method->member == "setInvalid") {
                    return parse_getSetHeaderValidity(method->expr, false);
                }

                if (method->member == "setValid") {
                    return parse_getSetHeaderValidity(method->expr, true);
                }
            ...
            if (method->expr->type->is<IR::Type_Stack>()) {
                if (method->member == "push_front") {
                    return parseGetStackPushPopFront(method->expr, call->arguments);
                }

                if (method->member == "pop_front") {
                    return parseGetStackPushPopFront(method->expr, call->arguments, false);
                }
...
bool ExprParseGetter::preorder(const IR::Mux *mux) {
    logParseGet(mux);

    if (!SymbolicEnv::isSymbolicValue(mux->e0)) {
        return parseGetToSubexpr(mux->e0, result, state, [mux](const Continuation::Parameter *v) {
...
bool ExprParseGetter::preorder(const IR::PathExpression *pathExpression) {
    logParseGet(pathExpression);
...
bool ExprParseGetter::preorder(const IR::P4ValueSet *valueSet) {
    logParseGet(valueSet);
...
bool ExprParseGetter::preorder(const IR::Operation_Binary *binary) {
    logParseGet(binary);

    if (!SymbolicEnv::isSymbolicValue(binary->left)) {
        return parseGetToSubexpr(binary->left, result, state,
    ...
    if (!SymbolicEnv::isSymbolicValue(binary->right)) {
        return parseGetToSubexpr(binary->right, result, state,
                             [binary](const Continuation::Parameter *v) {
                                 auto *result = binary->clone();
    ...
    return parseGetSymbolicValue(binary);
}

bool ExprParseGetter::preorder(const IR::Operation_Unary *unary) {
    logParseGet(unary);

    if (!SymbolicEnv::isSymbolicValue(unary->expr)) {
        return parseGetToSubexpr(unary->expr, result, state, [unary](const Continuation::Parameter *v) {
    ...
    return parseGetSymbolicValue(unary);
}

bool ExprParseGetter::preorder(const IR::SelectExpression *selectExpression) {
    logParseGet(selectExpression);
    ...
    if (!SymbolicEnv::isSymbolicValue(selectExpression->select)) {
        ...
        return parseGetToListSubexpr(selectExpression->select, result, state,
...
bool ExprParseGetter::preorder(const IR::ListExpression *listExpression) {
    if (!SymbolicEnv::isSymbolicValue(listExpression)) {
        // Evaluate the expression being selected.
        return parseGetToListSubexpr(listExpression, result, state,
    ...
    return parseGetSymbolicValue(listExpression);
}

bool ExprParseGetter::preorder(const IR::StructExpression *structExpression) {
    if (!SymbolicEnv::isSymbolicValue(structExpression)) {
        // Evaluate the expression being selected.
        return parseGetToStructSubexpr(structExpression, result, state,
    ...
    return parseGetSymbolicValue(structExpression);
}

bool ExprParseGetter::preorder(const IR::Slice *slice) {
    logParseGet(slice);

    if (!SymbolicEnv::isSymbolicValue(slice->e0)) {
        return parseGetToSubexpr(slice->e0, result, state, [slice](const Continuation::Parameter *v) {
    ...
    return parseGetSymbolicValue(slice);
}

void ExprParseGetter::parse_getNoMatch() { parseGetToException(Continuation::Exception::NoMatch); }
