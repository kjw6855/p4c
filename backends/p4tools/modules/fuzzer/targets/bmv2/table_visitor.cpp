...
const IR::Expression* BMv2_V1ModelTableVisitor::computeTargetMatchType(
...
void BMv2_V1ModelTableVisitor::evalTableActionProfile(
    ...
    for (size_t idx = 0; idx < tableActionList.size(); idx++) {
        ...
        const ActionCall ctrlPlaneActionCall(actionIndex, actionType, ctrlPlaneArgs);
        auto tableRule =
...
void BMv2_V1ModelTableVisitor::evalTableActionSelector(
    const std::vector<const IR::ActionListElement*>& tableActionList) {
    const auto* state = getVisitState();
...
bool BMv2_V1ModelTableVisitor::checkForActionProfile() {
...
bool BMv2_V1ModelTableVisitor::checkForActionSelector() {
    const auto* impl = table->properties->getProperty("implementation");
    if (impl == nullptr) {
        return false;
    }
...
void BMv2_V1ModelTableVisitor::checkTargetProperties(
    const std::vector<const IR::ActionListElement*>& /*tableActionList*/) {
...
void BMv2_V1ModelTableVisitor::evalTargetTable(
    const std::vector<const IR::ActionListElement*>& tableActionList) {
...
BMv2_V1ModelTableVisitor::BMv2_V1ModelTableVisitor(BMv2_V1ModelVisitStepper* stepper,
                                                   const IR::P4Table* table,
                                                   const TestCase& testCase)
    : TableVisitor(stepper, table, testCase) {}
