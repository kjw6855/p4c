#ifndef BACKENDS_P4TOOLS_MODULES_SYMBEX_TEST_SMALL_STEP_UTIL_H_
#define BACKENDS_P4TOOLS_MODULES_SYMBEX_TEST_SMALL_STEP_UTIL_H_
...
using Body = Symbex::Continuation::Body;
using Continuation = Symbex::Continuation;
using ExecutionState = Symbex::ExecutionState;
using NamespaceContext = NamespaceContext;
using Return = Symbex::Continuation::Return;
using SmallStepEvaluator = Symbex::SmallStepEvaluator;
using SymbexTarget = Symbex::SymbexTarget;
using Z3Solver = Z3Solver;

class SmallStepTest : public SymbexTest {
 public:
...
void stepAndExamineValue(const T *value, const CompilerResult &compilerResult) {
    ...
    const auto *progInfo = SymbexTarget::produceProgramInfo(compilerResult);
...
#endif /* BACKENDS_P4TOOLS_MODULES_SYMBEX_TEST_SMALL_STEP_UTIL_H_ */
