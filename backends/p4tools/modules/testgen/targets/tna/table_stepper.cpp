...
const IR::Expression *TnaTableStepper::computeTargetMatchType(
    ...
    if (keyProperties.matchType == TNAConstants::MATCH_KIND_OPT) {
    ...
    if (keyProperties.matchType == TNAConstants::MATCH_KIND_SELECTOR) {
        tnaProperties.actionSelectorKeys.emplace_back(keyExpr);
        return hitCondition;
    }
    // Ranges are not yet implemented for TNA STF tests.
    if (keyProperties.matchType == TNAConstants::MATCH_KIND_RANGE &&
...
void TnaTableStepper::evalTableActionProfile(
    ...
    for (const auto *action : tableActionList) {
        ...
        auto *actionProfile = new TnaActionProfile(*tnaProperties.actionProfile);
...
void TnaTableStepper::evalTableActionSelector(
    const std::vector<const IR::ActionListElement *> &tableActionList) {
    const auto *state = getExecutionState();
    ...
    for (const auto *action : tableActionList) {
        ...
        // Copy the previous action profile.
        auto *actionProfile = new TnaActionProfile(
            tnaProperties.actionSelector->getActionProfile()->getProfileDecl());
        // Synthesize arguments for the call based on the action parameters.
        const auto &parameters = actionType->parameters;
        auto *arguments = new IR::Vector<IR::Argument>();
        std::vector<ActionArg> ctrlPlaneArgs;
        ...
        auto *actionSelector = new TnaActionSelector(
            tnaProperties.actionSelector->getSelectorDecl(), actionProfile);
...
bool TnaTableStepper::checkForActionProfile() {
    ...
    if (testObject == nullptr) {
        ...
        tnaProperties.addProfileToState = true;
        tnaProperties.actionProfile = new TnaActionProfile(implDecl);
        return true;
    }
    tnaProperties.actionProfile = testObject->checkedTo<TnaActionProfile>();
    tnaProperties.addProfileToState = false;
    return true;
}

bool TnaTableStepper::checkForActionSelector() {
    const auto *impl = table->properties->getProperty("implementation");
    if (impl == nullptr) {
        return false;
    }
    ...
    if (testObject == nullptr) {
        ...
        // add the generated action profile.
        //tnaProperties.addProfileToState = true;
        //tnaProperties.actionProfile = new TnaActionProfile(selectorDecl);
        tnaProperties.actionSelector = new TnaActionSelector(selectorDecl,
                new TnaActionProfile(selectorDecl));
        return true;
    }
    tnaProperties.actionSelector = testObject->checkedTo<TnaActionSelector>();
    tnaProperties.addProfileToState = false;
    return true;
}

void TnaTableStepper::checkTargetProperties(
    const std::vector<const IR::ActionListElement *> & /*tableActionList*/) {
    ...
    if (checkForActionProfile()) {
        tnaProperties.implementaton = TableImplementation::profile;
        return;
    }
    ...
    if (checkForActionSelector()) {
        ...
        tnaProperties.implementaton = TableImplementation::selector;
        return;
    }
}

void TnaTableStepper::evalTargetTable(
    const std::vector<const IR::ActionListElement *> &tableActionList) {
    ...
    if (properties.tableIsImmutable) {
        tnaProperties.implementaton = TableImplementation::constant;
    }

    switch (tnaProperties.implementaton) {
...
TnaTableStepper::TnaTableStepper(TnaExprStepper *stepper,
