...
def parse_args():
    ...
    parser.add_argument(
        "rootdir", help="the root directory of " "the compiler source tree"
    )
    ...
    parser.add_argument(
        "-gdb", "--gdb", action="store_true", help="Run the compiler under gdb."
    )
    parser.add_argument(
        "-a",
    ...
    parser.add_argument(
        "--observation-log", dest="obs_log", help="save packet output to <file>"
    )
    parser.add_argument(
        "-tf",
        "--testFile",
...
if __name__ == "__main__":
    ...
    if not options.hasBMv2:
        reportError(
            "config.h indicates that BMv2 is not installed"
