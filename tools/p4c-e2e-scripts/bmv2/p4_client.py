...
class P4RuntimeClient:
    ...
    def _set_up_stream(self):
        ...
        # Wait until the gRPC channel is actually serviceable before opening the StreamChannel.
        # A StreamChannel opened on a server whose socket is bound but whose gRPC/HTTP2 stack is
        # not yet serving busy-loops at ~99% CPU instead of blocking — the startup race behind the
        # corpus "no divergence" replay runs. channel_ready_future blocks cleanly until READY.
        # This complements Bmv2Process._wait_for_grpc and also guards direct/reconnect callers.
        # Raise (not sys.exit) so a caller can record an error and move to the next test.
        try:
            grpc.channel_ready_future(self.channel).result(timeout=15)
        except grpc.FutureTimeoutError:
            raise RuntimeError(
                f"gRPC channel to {self.grpc_address} not ready after 15s") from None
    ...
    def _handshake(self, attempts=3):
        # Retry the arbitration a few times before giving up: a not-yet-warm server can miss the
        # first reply. Raise (not sys.exit) on terminal failure so the caller can recover.
        for attempt in range(1, attempts + 1):
            ...
            if rep is not None:
                ...
                return
            LOGGER.warning("no arbitration reply from server (attempt %d/%d)", attempt, attempts)
        raise RuntimeError("failed to establish P4Runtime session (no arbitration reply)")
    ...
    def tear_down(self):
        """Close the StreamChannel and the gRPC channel. Idempotent."""
        stream = getattr(self, "stream", None)
        if stream is not None:
            # Cancel the RPC so the server reaps this StreamChannel promptly. A lingering
            # half-open stream (e.g. from a client that did not close cleanly) can wedge the
            # next client's mastership arbitration.
            try:
                stream.cancel()
            except Exception:
                pass
