...
class BackendDriver:
    ...
    def process_command_line_options(self, opts):
        ...
        self.add_command_option(
            "preprocessor", "-C -undef -nostdinc -x assembler-with-cpp"
        )
        ...
        if opts.language == "p4-16":
            ...
            self.add_command_option(
                "compiler", "-I {}".format(os.environ["P4C_16_INCLUDE_PATH"])
            )
        else:
            self.add_command_option(
            ...
            self.add_command_option(
                "compiler", "-I {}".format(os.environ["P4C_14_INCLUDE_PATH"])
            )
        ...
        if opts.p4runtime_file:
            ...
            self.add_command_option(
                "compiler", "--p4runtime-file {}".format(opts.p4runtime_file)
            )
            self.add_command_option(
        ...
        if opts.p4runtime_files:
            self.add_command_option(
                "compiler", "--p4runtime-files {}".format(opts.p4runtime_files)
            )
        ...
        if os.environ["P4C_BUILD_TYPE"] == "DEVELOPER":
            ...
            if opts.passes:
                self.add_command_option(
                    "compiler", "--top4 {}".format(",".join(opts.passes))
                )
            if opts.debug:
            ...
            if opts.json_source:
                self.add_command_option(
                    "compiler", "--fromJSON {}".format(opts.json_source)
                )
    ...
    def run(self):
        ...
        if (
            ...
            and not self._run_preprocessor_only
        ):
