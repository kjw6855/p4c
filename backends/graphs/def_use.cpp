...
bool ComputeDefUse::preorder(const IR::P4Control *c) {
    ...
    for (auto g : controlGraphsArray) {
        auto g_name = boost::get_property(*g, boost::graph_name);
        if (c->name.name.string() == g_name) {
            curG = g;
            break;
        }
    }

    // Stop creating DefUse
    if (curG == nullptr)
        return false;
...
void dump(const P4::graphs::ComputeDefUse::loc_t &p) { std::cout << p << std::endl; }
void dump(const hvec_set<const P4::graphs::ComputeDefUse::loc_t *> &p) { std::cout << p << std::endl; }
void dump(const P4::graphs::ComputeDefUse &du) { std::cout << du << std::endl; }
void dump(const P4::graphs::ComputeDefUse *du) { std::cout << *du << std::endl; }
