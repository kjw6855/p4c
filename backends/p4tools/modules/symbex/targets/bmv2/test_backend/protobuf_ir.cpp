...
std::optional<std::string> ProtobufIr::checkForP4RuntimeTranslationAnnotation(
    ...
    SYMBEX_UNIMPLEMENTED("Unsupported @p4runtime_translation token %1%", targetValue);
}
...
std::string ProtobufIr::getFormatOfNode(const IR::IAnnotated *node) {
    ...
    SYMBEX_UNIMPLEMENTED("Unsupported @format string %1%", annotationFormatString);
}

std::string ProtobufIr::getTestCaseTemplate() {
    static std::string TEST_CASE(
        R"""(# proto-file: symbex_ir.proto
...
std::string ProtobufIr::formatNetworkValue(const std::string &type, const IR::Expression *value) {
    ...
    if (type == "str") {
        ...
        SYMBEX_UNIMPLEMENTED("Unsupported string format value \"%1%\".", value);
    }
    ...
    SYMBEX_UNIMPLEMENTED("Unsupported network value type %1%", type);
}
