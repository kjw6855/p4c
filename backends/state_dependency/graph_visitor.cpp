...
const char *GraphVisitor::getType(const VertexFlags &v_flags) {
    return vertexFlagsToString(v_flags).c_str();
}

const char *GraphVisitor::getPrevType(const PrevType &prev_type) {
    switch (prev_type) {
...
void GraphVisitor::forLoopJson(std::vector<Graph *> &graphsArray, PrevType node_type) {
    for (auto g : graphsArray) {
        ...
        for (auto &vit = vertices.first; vit != vertices.second; ++vit) {
            ...
            node->emplace("type", getType(vinfo.flags));
            node->emplace("type_enum", (unsigned)vinfo.flags);
        }
...
void GraphVisitor::forLoopFullGraph(std::vector<Graph *> &graphsArray, fullGraphOpts *opts,
                                     PrevType prev_type) {
    ...
    for (auto g_ : graphsArray) {
        ...
        if (g_->m_global_vertex.size() == 2) {
            add_vertex(cstring("Empty body"), VertexFlags::EMPTY);
        } else {
...
void GraphVisitor::process(std::vector<Graph *> &controlGraphsArray,
                            std::vector<Graph *> &parserGraphsArray) {
    if (graphs) {
        for (auto g : controlGraphsArray) {
            GraphAttributeSetter()(*g, varVis, varEdgeVis);
