#include "backends/p4tools/modules/testgen/targets/bmv2/expr_parse_getter.h"
...
std::string Bmv2V1ModelExprParseGetter::getClassName() { return "Bmv2V1ModelExprParseGetter"; }

bool Bmv2V1ModelExprParseGetter::isPartOfFieldList(const IR::StructField *field,
...
void Bmv2V1ModelExprParseGetter::resetPreservingFieldList(ExecutionState &nextState,
...
void Bmv2V1ModelExprParseGetter::processClone(const ExecutionState &state,
                                          SmallParseGetEvaluator::Result &result) {
    ...
    // tests?
    // TODO: Circle back to this once we have modifiable state in extern parse_getters.
...
void Bmv2V1ModelExprParseGetter::processRecirculate(const ExecutionState &state,
                                                SmallParseGetEvaluator::Result &result) {
...
Bmv2V1ModelExprParseGetter::Bmv2V1ModelExprParseGetter(ExecutionState &state,
                                               const ProgramInfo &programInfo)
    : ExprParseGetter(state, programInfo) {}

#if 0
void Bmv2V1ModelExprParseGetter::evalExternMethodCall(const IR::MethodCallExpression *call,
        ...
        [](const IR::MethodCallExpression *call, const IR::Expression * /*receiver*/,
           ...
           SmallParseGetEvaluator::Result &result) {
            ...
            if (!SymbolicEnv::isSymbolicValue(cond)) {
                ...
                parseGetToSubexpr(cond, result, state, [call](const Continuation::Parameter *v) {
    ...
    static const ExternMethodImpls EXTERN_METHOD_IMPLS({
        ...
        {"*method.mark_to_drop",
         ...
         [](const IR::MethodCallExpression * /*call*/, const IR::Expression * /*receiver*/,
            ...
            const ExecutionState &state, SmallParseGetEvaluator::Result &result) {
             auto &nextState = state.clone();
        ...
        {"*method.hash",
         ...
         [this](const IR::MethodCallExpression *call, const IR::Expression *receiver, IR::ID &name,
             ...
             for (size_t idx = 1; idx < args->size(); ++idx) {
                 ...
                 // TODO: Frontload this in the expression parse_getter for method call expressions.
                 if (!SymbolicEnv::isSymbolicValue(argExpr)) {
                     // Evaluate the condition.
                     parseGetToSubexpr(argExpr, result, state,
         ...
         [](const IR::MethodCallExpression *call, const IR::Expression *receiver,
            ...
            const ExecutionState &state, SmallParseGetEvaluator::Result &result) {
             ...
             if (!SymbolicEnv::isSymbolicValue(index)) {
                 // Evaluate the condition.
                 parseGetToSubexpr(index, result, state, [call](const Continuation::Parameter *v) {
                     auto *clonedCall = call->clone();
                     auto *arguments = clonedCall->arguments->clone();
    ...
    if (!EXTERN_METHOD_IMPLS.exec(call, receiver, name, args, state, result)) {
        ExprParseGetter::evalExternMethodCall(call, receiver, name, args, state);
    }
} // NOLINT
#endif

/*
bool Bmv2V1ModelExprParseGetter::preorder(const IR::P4Table *table) {
    // Delegate to the tableParseGetter.
    Bmv2V1ModelTableParseGetter tableParseGetter(this, table);

    return tableParseGetter.eval();
}
*/
}  // namespace P4Tools::P4Testgen::Bmv2
