...
TnaCmdVisitor::TnaCmdVisitor(ExecutionState &state,
                                             const ProgramInfo &programInfo, TestCase &testCase)
    : CmdVisitor(state, programInfo, testCase) {}

const TnaProgramInfo &TnaCmdVisitor::getProgramInfo() const {
    return *CmdVisitor::getProgramInfo().checkedTo<TnaProgramInfo>();
}

void TnaCmdVisitor::initializeTargetEnvironment(ExecutionState &nextState, TestCase& testCase) const {
...
std::optional<const Constraint *> TnaCmdVisitor::startParserImpl(
...
std::map<Continuation::Exception, Continuation> TnaCmdVisitor::getExceptionHandlers(
