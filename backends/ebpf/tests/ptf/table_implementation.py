...
class SimpleActionProfilePSATest(P4EbpfTest):
    ...
    def runTest(self):
        pkt = testutils.simple_ip_packet(
            eth_src="11:22:33:44:55:66", eth_dst="22:33:44:55:66:77"
        )
...
class ActionProfileTwoTablesSameInstancePSATest(P4EbpfTest):
    ...
    def runTest(self):
        ...
        pkt = testutils.simple_ip_packet(
            eth_src="AA:BB:CC:DD:EE:FF", eth_dst="22:33:44:55:66:77"
        )
        testutils.send_packet(self, PORT0, pkt)
        pkt[Ether].type = 0x1122
        testutils.verify_packet_any_port(self, pkt, PTF_PORTS)
...
class ActionProfileActionRunPSATest(P4EbpfTest):
    ...
    def runTest(self):
        ...
        ref2 = self.action_profile_add_action(
            ap="MyIC_ap", action=1, data=["AA:BB:CC:DD:EE:FF"]
        )
...
class ActionSelectorTest(P4EbpfTest):
    ...
    def create_actions(self, selector):
        for i in range(1, 7):
            ...
            ref = self.action_selector_add_action(
                selector, action=1, data=[DP_PORTS[i - 1]]
            )
            if i != ref:
                self.fail(
                    "Invalid member reference: expected {}, got {}".format(i, ref)
                )

    def group_add_members(self, selector, gid, member_refs):
        for m in member_refs:
            self.action_selector_add_member_to_group(
                selector=selector, group_ref=gid, member_ref=m
            )
    ...
    def create_default_rule_set(self, table, selector):
        ...
        self.group_add_members(
            selector=selector, gid=self.group_id, member_refs=[4, 5, 6]
        )
...
class SimpleActionSelectorPSATest(ActionSelectorTest):
    ...
    def runTest(self):
        ...
        # member reference
        pkt = testutils.simple_ip_packet(
            eth_src="02:22:33:44:55:66", eth_dst="22:33:44:55:66:77"
        )
        testutils.send_packet(self, PORT0, pkt)
        testutils.verify_packet(self, pkt, PORT1)
        ...
        pkt = testutils.simple_ip_packet(
            eth_src="07:22:33:44:55:66", eth_dst="22:33:44:55:66:77"
        )
        testutils.send_packet(self, PORT0, pkt)
...
class ActionSelectorEmptyGroupActionPSATest(ActionSelectorTest):
    ...
    def runTest(self):
        ...
        pkt = testutils.simple_ip_packet(
            eth_src="08:22:33:44:55:66", eth_dst="22:33:44:55:66:77"
        )
        testutils.send_packet(self, PORT0, pkt)
        testutils.verify_no_other_packets(self)
...
class ActionSelectorMultipleSelectorsTwoTablesPSATest(ActionSelectorTest):
    ...
    def runTest(self):
        ...
        testutils.send_packet(self, PORT0, pkt)
        (port, _) = testutils.verify_packet_any_port(
            self, pkt, self.default_group_ports
        )
...
class ActionSelectorLPMTablePSATest(ActionSelectorTest):
    ...
    def runTest(self):
        ...
        pkt = testutils.simple_ip_packet(
            eth_src="00:22:07:44:55:66", eth_dst="22:33:44:55:66:77"
        )
        testutils.send_packet(self, PORT0, pkt)
        testutils.verify_packet_any_port(self, pkt, self.default_group_ports)

        pkt = testutils.simple_ip_packet(
            eth_src="00:22:02:44:55:66", eth_dst="22:33:44:55:66:77"
        )
        testutils.send_packet(self, PORT0, pkt)
        testutils.verify_packet(self, pkt, PORT1)
...
class ActionSelectorActionRunPSATest(ActionSelectorTest):
    ...
    def runTest(self):
        ref1 = self.action_selector_add_action(
            selector="MyIC_as", action="MyIC_fwd", data=[5]
        )
