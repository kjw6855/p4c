...
inja::json PTF::getClone(const TestObjectMap &cloneSpecs) {
    ...
    for (auto cloneSpecTuple : cloneSpecs) {
        ...
        const auto *cloneSpec = cloneSpecTuple.second->checkedTo<TnaCloneSpec>();
...
inja::json::array_t PTF::getMeter(const TestObjectMap &meterValues) {
    ...
    for (auto meterValueInfoTuple : meterValues) {
        const auto *meterValue = meterValueInfoTuple.second->checkedTo<TnaMeterValue>();
...
inja::json PTF::getControlPlane(const TestSpec *testSpec) {
    ...
    for (const auto &testObject : tables) {
        ...
        checkForTableActionProfile<TnaActionProfile, TnaActionSelector>(
    ...
    collectActionProfileDeclarations<TnaActionProfile>(testSpec, controlPlaneJson, apAsMap);
